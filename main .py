...
def api_get(endpoint: str, params: dict, timeout: int = 10) -> dict:
    ...
    _api_call_count += 1
    if _api_call_count > 95:
        logger.warning(f"⚠️ API quota near limit: {_api_call_count} calls today")
    try:
...
def get_team_form(team_id: int, league_id: int, last: int = 10) -> dict:
    """Get detailed team form with extended stats — cached 3 hours"""
    ...
    cache_set(cache_key, result, 10800)  # 3 hours — form doesn't change during a day
    return result
...
async def get_today_fixtures():
    ...
    Increased timeout + retry pass ensures slow leagues aren't missed.
    Only caches when all leagues responded successfully."""
    today_str = datetime.now().strftime("%Y-%m-%d")
    ...
    def fetch_league_today(league: dict, timeout: int = 12) -> list:
        """Fetch today's fixtures for one league. Returns [] on failure."""
        try:
            data = api_get("fixtures", {
            ...
            return None  # None = failed (vs [] = genuinely no games today)

    loop = asyncio.get_event_loop()

    # ── Pass 1: fetch all leagues with 12s timeout ──
    ...
    # Find which leagues failed (returned None or exception)
    failed_leagues = [LEAGUES[i] for i, r in enumerate(results)
                      if r is None or isinstance(r, Exception)]

    # ── Pass 2: retry failed leagues once with longer timeout ──
    if failed_leagues:
        logger.warning(f"⚠️  /today: {len(failed_leagues)} leagues timed out — retrying: {[l['code'] for l in failed_leagues]}")
        with ThreadPoolExecutor(max_workers=len(failed_leagues)) as executor:
            retry_futures = [loop.run_in_executor(executor, fetch_league_today, lg, 20)
    ...
    still_failed = [LEAGUES[i] for i, r in enumerate(results)
                    if r is None or isinstance(r, Exception)]
    result = {
        "matches": all_matches,
    ...
    # Only cache if all leagues responded (or max 1 failed — could genuinely have no games)
    if all_matches and len(still_failed) <= 1:
        cache_set(cache_key, result, 1200)  # 20 minutes
    elif all_matches:
        cache_set(cache_key, result, 120)   # partial result — short 2-min cache so it retries soon
...
async def refresh_today():
    """Bust the today cache so next load re-fetches everything fresh."""
    today_str = datetime.now().strftime("%Y-%m-%d")
    cache_key = f"today_{today_str}"
    if cache_key in _cache:
        del _cache[cache_key]
    return {"ok": True, "message": "Today cache cleared — next load will fetch fresh data"}
...
async def get_h2h(team1_id: int, team2_id: int):
    ...
    if matches:
        cache_set(cache_key, result, 10800)  # 3 hours — h2h is historical
    return result
...
async def get_today_acca():
    ...
    async def fetch_picks(risk: str, endpoint: str = "form") -> list:
        try:
            if endpoint == "rank":
                # Reuse the table-rank logic inline
                req = AccaRequest(selections=8, risk=risk, leagues=leagues, market="winner")
    ...
    form_picks, rank_picks, value_picks = await asyncio.gather(
        fetch_picks("balanced", "form"),
        fetch_picks("balanced", "rank"),
        fetch_picks("risky",    "form"),
    )
...
async def generate_acca(request: AccaRequest):
    try:
        ...
        today = datetime.now().date()
        # Always cap to 3 days max — no today_only restriction
        cutoff = today + timedelta(days=3)
        all_picks = []
        seen = set()
        ...
        def fetch_league_picks(lc: str) -> list:
            ...
            try:
                ...
                for f in fixtures:
                    ...
                    if pick and pick["confidence"] >= 45:  # lowered from 55
                        picks.append(pick)
            except Exception as e:
