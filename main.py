...
def score_inplay_markets(fixture_id: int, home: str, away: str,
    ...
    """
    Score a live match for in-play markets.
    Tightened rules:
    - Match Winner: 75+ min, DRAW only, one team clearly dominant
    - Goals markets: tiered by xG threshold (2.5 or 1.5 depending on minute)
    - BTTS: both teams must have xG, one yet to score
    - Comeback: HT window, losing team leads xG
    """
    alerts = []
    hs  = team_stats.get(home, {})
    as_ = team_stats.get(away, {})
    ...
    a_poss    = 100 - h_poss
    h_shots   = int(hs.get("shots_on_goal") or hs.get("total_shots") or 0)
    a_shots   = int(as_.get("shots_on_goal") or as_.get("total_shots") or 0)
    h_corners = int(hs.get("corner_kicks") or 0)
    a_corners = int(as_.get("corner_kicks") or 0)

    score_str = f"{home_score}-{away_score}"
    is_draw   = home_score == away_score

    # ── 1. MATCH WINNER (Late game, draw, dominant team) ──────────────────
    # Only fire 75+ min, game MUST be 0-0 or level draw, one side clearly dominant
    # NEVER fire if either team is already winning
    if is_draw and minute >= 75 and total_goals <= 2:
        winner = None
        dom_xg  = 0.0
        dom_poss = 0.0
        dom_shots = 0

        if (h_xg > a_xg + 0.9        # strong xG edge required
            and h_poss >= 58          # clear possession dominance
            and h_shots > a_shots     # must have more shots on target
        ):
            winner    = home
            dom_xg    = h_xg
            dom_poss  = h_poss
        ...
        if winner:
            remaining = 90 - minute
            conf = min(88, int(68 + abs(xg_gap) * 12 + (dom_poss - 50) * 0.4))
            alerts.append({
                "market": "Match Winner",
                "team": winner,
                "fixture_id": fixture_id,
                ...
                "reason": (
                    f"{winner} dominant — xG {dom_xg:.2f} vs {(a_xg if winner==home else h_xg):.2f}, "
                    f"{dom_poss:.0f}% poss, {dom_shots} shots on target. "
                    f"Score still {score_str} at {minute}' — {remaining} mins to break"
                ),
                "confidence": conf,
            })

    # ── 2. GOALS MARKETS — tiered xG thresholds ───────────────────────────
    # Early/mid game: need 2.5 xG to call Over 2.5 goals
    # Late game: 1.5 xG enough to call Over 1.5 goals
    if minute < 45:
        # First half: need 3.0+ xG to call Over 2.5 — very high bar in first half
        if total_xg >= 3.0 and total_goals < 2:
            alerts.append({
                ...
                "minute": minute, "league": league,
                "reason": (
                    f"Combined xG already {total_xg:.2f} with {total_goals} goal(s) — "
                    f"xG significantly outpacing scoreline, more goals likely"
                ),
                "confidence": min(88, int(62 + (total_xg - 2.5) * 14)),
            })

    elif 45 <= minute < 70:
        # Second half early: need 2.5 xG total still
        if total_xg >= 2.5 and total_goals < 3:
            remaining = 90 - minute
            alerts.append({
                "market": "Over 2.5 Goals",
                "fixture_id": fixture_id,
                ...
                "minute": minute, "league": league,
                "reason": (
                    f"xG {total_xg:.2f} vs only {total_goals} goal(s) — "
                    f"{remaining} mins left, xG strongly suggests more goals coming"
                ),
                "confidence": min(90, int(65 + (total_xg - 2.5) * 12)),
            })

    elif 70 <= minute < 85:
        # Late game: lower bar — 1.5 xG unscored
        xg_unscored = round(total_xg - total_goals, 2)
        if xg_unscored >= 1.5 and total_goals < 2:
            remaining = 90 - minute
            alerts.append({
                ...
                "minute": minute, "league": league,
                "reason": (
                    f"{xg_unscored:.1f} xG unaccounted for — only {total_goals} goal(s) in {minute}' "
                    f"with {remaining} mins left. xG says more goals due"
                ),
                "confidence": min(87, int(65 + xg_unscored * 10)),
            })

    # ── 3. BTTS (Both Teams to Score) ──────────────────────────────────────
    # One team scoreless but both generating good xG
    if 30 <= minute < 75 and (home_score == 0 or away_score == 0):
        if h_xg >= 1.0 and a_xg >= 1.0:
            scoreless = home if home_score == 0 else away
            scored    = away if home_score == 0 else home
            conf = min(84, int(58 + (h_xg + a_xg) * 7))
            alerts.append({
                ...
                "reason": (
                    ...
                    f"{scoreless} yet to convert with {90-minute} mins remaining"
                ),
                "confidence": conf,
            })

    # ── 4. HT/FT COMEBACK ──────────────────────────────────────────────────
    # Around HT: team losing but dominates xG
    if 40 <= minute <= 52:
        if home_score < away_score and h_xg > a_xg + 0.5:
            alerts.append({
                ...
                "minute": minute, "league": league,
                "reason": (
                    f"{home} losing {score_str} but leads xG {h_xg:.2f} vs {a_xg:.2f} — "
                    f"dominant team, 2nd half comeback strongly likely"
                ),
                "confidence": 74,
            })
        elif away_score < home_score and a_xg > h_xg + 0.5:
            alerts.append({
                "market": "HT/FT Value — Comeback",
                "team": away,
                "fixture_id": fixture_id,
                ...
                "minute": minute, "league": league,
                "reason": (
                    f"{away} losing {score_str} but leads xG {a_xg:.2f} vs {h_xg:.2f} — "
                    f"dominant team, 2nd half comeback strongly likely"
                ),
                "confidence": 74,
            })
        ...
        try:
            ...
            for i, alert in enumerate(alerts_found):
                ...
                alerted_details[fid] = {
                    ...
                    "original_msg": build_free_message(alert),
                    "pro_msg": build_telegram_message(alert),
                    "free_mid": free_mid,
                    "pro_mid": pro_mid if has_pro else 0,
                    "result_posted": False,
                    "fixture_id": fid,
                    "home_score_at_alert": alert.get("score", "0-0").split("-")[0].strip(),
                    "away_score_at_alert": alert.get("score", "0-0").split("-")[-1].strip(),
                    "last_known_score": alert.get("score", "0-0"),
                    "goal_update_posted": False,
                }
...
async def admin_gift_pro(request: dict):
    ...
    logger.info(f"Gift auth: check_len={len(check)} admin_len={len(admin_pw)} match={check==admin_pw}")
    if not admin_pw:
        return {"ok": False, "error": "ADMIN_PASSWORD not configured in Railway"}
    if check != admin_pw:
        return {"ok": False, "error": f"Unauthorised — received {len(check)} chars, expected {len(admin_pw)} chars"}
...
def _load_user_db():
    """Load user db from file on startup (survives code changes, not full container rebuilds)."""
    global _user_db
    try:
        import json
        if os.path.exists(_USER_DB_FILE):
            ...
            logger.info(f"✅ Loaded {len(_user_db)} users from disk")
    except Exception as e:
        logger.warning(f"Could not load user db: {e}")

def _save_user_db():
    """Persist user db to file."""
    try:
        import json
        with open(_USER_DB_FILE, 'w') as f:
            json.dump(_user_db, f)
    except Exception as e:
...
async def _handle_stripe_payment(event: dict):
    ...
    # Send the welcome email with the code
