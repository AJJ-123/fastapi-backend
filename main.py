...
# old /sheet/prematch removed — see protected version below

# old /sheet/steam removed — see protected version below
...
async def download_steam_sheet_protected(email: str = ""):
    ...
    try:
        ...
        HDRS=["Fixture","League","7am O2.5","5m O2.5","O2.5 Move","O2.5 %","O2.5 Steam",
              ...
              "LTD?","LU1.5?","LFHU0.5?"]
        N=len(HDRS)
        out=Workbook(); ws=out.active; ws.title="Steam Detection"
        ws.sheet_view.showGridLines=False; ws.freeze_panes="A4"
        ws.merge_cells(f"A1:{get_column_letter(N)}1")
        ws["A1"]=f"AGD Sports Trading  —  Steam Detection  |  {today.strftime('%A %d %B %Y')}  |  PRIVATE"
        ws["A1"].font=Font(name="Calibri",size=11,bold=True,color=AG_GOLD)
        ws["A1"].fill=fill(AG_BLACK); ws["A1"].alignment=lft()
        for ci in range(2,N+1): ws.cell(row=1,column=ci).fill=fill(AG_BLACK)
        ws.row_dimensions[2].height=14
        ws.cell(row=2,column=1).value="System 1 LTD: Draw+O2.5 steam  ·  System 2 LU1.5: O2.5 steams 3%+  ·  System 3 LFHU0.5: O1.5 steams 3%+"
        ...
        for i,hdr in enumerate(HDRS,1):
            c2=ws.cell(row=3,column=i,value=hdr)
            if "7am" in hdr: bg,fc="FF003366","FFAACCFF"
            elif "5m" in hdr: bg,fc="FF006633","FFAAFFCC"
            elif "Move" in hdr or "%" in hdr: bg,fc="FF332600",AG_GOLD
            elif "Steam" in hdr: bg,fc=STEAM_HDR,BLACK
            elif hdr in("LTD?","LU1.5?","LFHU0.5?"): bg,fc="FF001A00","FF66FF66"
            elif hdr=="Fixture": bg,fc=AG_BLACK,AG_GOLD
            else: bg,fc=AG_BLACK,MUTED
            c2.font=Font(name="Calibri",size=8,bold=True,color=fc)
            c2.fill=fill(bg); c2.alignment=ctr(); c2.border=bdr()
        THRESH=3.0; rows_written=0
        for fkey,fdata in _steam_prices.items():
            ...
            o25_steam=o25_s=="STEAM"; draw_steam=draw_s=="STEAM"; o15_steam=o15_s=="STEAM"
            ...
            ri=rows_written+4
            row_vals=[fkey,league,o25_7,o25_5,o25_m,f"{o25_p}%" if o25_p else "",o25_s,
            ...
            for ci,val in enumerate(row_vals[:N],1):
                ...
                if ci==1: c2.font=Font(name="Calibri",size=8,bold=True,color=AG_GOLD if has_steam else WHITE);c2.alignment=lft()
                elif "STEAM" in sv or "LTD" in sv or "LU1.5" in sv or "LFHU0.5" in sv:
                    c2.font=Font(name="Calibri",size=8,bold=True,color=GREEN);c2.alignment=ctr()
                elif "Move" in HDRS[ci-1]:
                    try:
                        v=float(sv);c2.font=Font(name="Calibri",size=8,bold=True,color=GREEN if v>0 else(RED if v<0 else MUTED))
                    except: c2.font=Font(name="Calibri",size=8,color=WHITE)
                    c2.alignment=ctr()
                else:
                    c2.font=Font(name="Calibri",size=8,color=WHITE);c2.alignment=ctr()
                c2.border=bdr()
            rows_written+=1
        cw={1:28,2:20}
