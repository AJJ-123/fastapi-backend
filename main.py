...
async def get_live_odds(fixture_id: int):
    """In-play odds for a live fixture — cached 60 seconds."""
    cache_key = f"live_odds_{fixture_id}"
    cached = cache_get(cache_key)
    if cached:
        return cached
    # Try live odds endpoint first
    data = api_get("odds/live", {"fixture": fixture_id}, timeout=6)
    response_list = data.get("response", []) if data else []
    best = {"home": {"odds": 0, "bookmaker": ""}, "draw": {"odds": 0, "bookmaker": ""}, "away": {"odds": 0, "bookmaker": ""}}
    if response_list:
        for bm in response_list[0].get("odds", [])[:6]:
            bm_name = bm.get("name","")
            for bet in bm.get("bets", []):
                if "Winner" in bet.get("name","") or "Match" in bet.get("name",""):
    ...
    result = {"fixture_id": fixture_id, "best_odds": best, "live": bool(response_list)}
