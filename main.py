        ...
        try:
            ...
            for m in live_matches:
                try:
                    ...
                    # Cache stats per fixture for 60s — reduces API calls, keeps data fresh
                    ...
                    if cached_stats:
                        ...
                        for td in stats_raw:
                            ...
                            if not ts.get("expected_goals") or ts.get("expected_goals") == 0:
                                shots_on = ts.get("shots_on_goal", 0) or 0
                                total_shots = ts.get("total_shots", 0) or 0
                                dangerous = ts.get("dangerous_attacks", 0) or 0
                                # Simple xG proxy: shots on target * 0.33 + other shots * 0.05
                                proxy_xg = round((shots_on * 0.33) + ((total_shots - shots_on) * 0.05) + (dangerous * 0.01), 2)
                                if proxy_xg > 0:
                                    ts["expected_goals"] = proxy_xg
                                    ts["xg_is_proxy"] = True
                            team_stats[tname] = ts
                ...
                try:
                    ...
                    h_dangerous = int(home_ts.get("dangerous_attacks") or 0)
                    a_dangerous = int(away_ts.get("dangerous_attacks") or 0)
                    ...
                    if h_xg > a_xg * 1.3:
                        ...
                        dom_xg = h_xg
                        dom_shots = h_shots
                        dom_corners = h_corners
                    elif a_xg > h_xg * 1.3:
                        dominant = m["away"]
                        dom_xg = a_xg
                        dom_shots = a_shots
                        dom_corners = a_corners
                    else:
                        dominant = None
                        dom_xg = max(h_xg, a_xg)
                        dom_shots = max(h_shots, a_shots)
                        dom_corners = max(h_corners, a_corners)

                    # Fire conditions — relaxed slightly for proxy xG
                    threshold = 1.2 if is_proxy else 1.0
                    should_fire = (
                        live_min >= 40 and
                        xg_unscored >= threshold and
                        total_goals < 4 and
                        total_xg > 0
                    )
                    ...
                    if should_fire:
                        ...
                        if not cache_get(gd_key):
                            ...
                            dom_line = f"\n  🎯 <b>{dominant}</b> looking most likely to score" if dominant else ""

                            # Build signal list
                            signals = []
                            if xg_unscored >= 1.0:
                                signals.append(f"xG {total_xg:.2f} vs {total_goals} goals — {xg_unscored:.2f} unscored")
                            if total_corners >= 6:
                                signals.append(f"{total_corners} corners — set piece pressure building")
                            if dominant and dom_shots >= 4:
                                signals.append(f"{dominant} {dom_shots} shots on target")
                            if dominant and dom_corners >= 4:
                                signals.append(f"{dominant} {dom_corners} corners — dominant")
                            if h_dangerous + a_dangerous >= 20:
                                signals.append(f"{h_dangerous + a_dangerous} dangerous attacks combined")
                            ...
                            gd_msg = (
                                ...
                                f"{'━'*22}\n"
                                f"📈 <b>Signals:</b>\n{signals_text}\n\n"
                                f"⚡ xG pressure building — goal expected soon\n"
                                f"{'━'*22}\n"
                            ...
                            if has_pro:
                                ...
                                if sent:
                                    cache_set(gd_key, True, 1800)  # 30 min cooldown per score
                                    logger.info(f"✅ Goal Due alert: {m['home']} vs {m['away']} {live_min}' xG={xg_unscored} proxy={is_proxy}")
                                    _paper_bot_place_bet(
                                        alert_type="Goal Due",
                                        match=f"{m['home']} vs {m['away']}",
                                        league=m["league"],
