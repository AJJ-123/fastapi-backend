...
# ══════════════════════════════════════════════════════════════
# PERSISTENT DATABASE — SQLite (survives Railway deploys)
# ══════════════════════════════════════════════════════════════
import sqlite3
import json as _json

# Use DATABASE_URL for postgres if set, else SQLite
_DB_FILE = os.getenv("DB_PATH", "/app/accagenius.db")
...
def _init_db():
    ...
    with _get_db() as db:
        ...
        """)
    print("✅ SQLite DB initialised at", _DB_FILE)
...
def db_load_all_users() -> dict:
    try:
        with _get_db() as db:
            rows = db.execute("SELECT email, data FROM users").fetchall()
    ...
    try:
        with _get_db() as db:
            db.execute(
                ...
                (str(acca.get("id")), acca.get("email",""), _json.dumps(acca),
                 acca.get("created_at", datetime.now().isoformat()))
            )
    except Exception as e:
...
def db_get_accas(email: str) -> list:
    try:
        with _get_db() as db:
            rows = db.execute(
                "SELECT data FROM saved_accas WHERE email=? ORDER BY created_at DESC",
                (email.lower(),)
            ).fetchall()
    ...
    try:
        with _get_db() as db:
            db.execute(
                ...
                (code, email, datetime.now().isoformat() if email else None, source)
            )
    except Exception as e:
...
def db_load_codes() -> tuple:
    ...
    try:
        with _get_db() as db:
            rows = db.execute("SELECT code, email, used_at, source FROM codes").fetchall()
