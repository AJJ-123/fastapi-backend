...
def score_inplay_markets(fixture_id: int, home: str, away: str,
    ...
    """
    Score a live match for ALL in-play markets beyond just match winner.
    Returns list of alert dicts for each triggered market.
    """
    alerts = []
    hs = team_stats.get(home, {})
    as_ = team_stats.get(away, {})
    ...
    xg_over   = round(total_xg - total_goals, 2)
    ...
    # ── 1. DRAW NO BET (DNB) ──
    # Team dominating xG but score is level — DNB value
    if home_score == away_score and minute >= 30:
        dnb_team = None
        if h_xg > a_xg + 0.5 and h_poss > 55:
            dnb_team = home
        elif a_xg > h_xg + 0.5 and a_poss > 55:
            dnb_team = away
        if dnb_team:
            alerts.append({
                "market": "Draw No Bet",
                ...
                "reason": f"xG dominance {h_xg:.1f}-{a_xg:.1f}, {h_poss:.0f}% possession — score still level",
                "confidence": 75,
            })

    # ── 2. OVER X.5 GOALS (remaining) ──
    if minute < 75:
        remaining = 90 - minute
        proj_goals = total_goals + (xg_over * (remaining / 45))
        if xg_over >= 0.8 and proj_goals > total_goals + 0.5:
            market = "Over 0.5 Goals" if total_goals == 0 else "Next Goal"
            alerts.append({
                "market": market,
                "fixture_id": fixture_id,
                "home": home, "away": away,
                "score": f"{home_score}-{away_score}",
                "minute": minute, "league": league,
                "reason": f"xG {total_xg:.1f} vs {total_goals} goals — {xg_over:.1f} xG unfinished with {remaining}min left",
                "confidence": min(92, int(65 + xg_over * 15)),
            })

    # ── 3. BTTS (Both Teams to Score) ──
    if home_score == 0 or away_score == 0:
        # One team hasn't scored yet but has good xG
        if h_xg > 0.7 and a_xg > 0.7 and minute < 70:
            ...
            alerts.append({
                ...
                "score": f"{home_score}-{away_score}",
                "minute": minute, "league": league,
                "reason": f"Both teams generating xG ({h_xg:.1f} & {a_xg:.1f}) — {scoreless} yet to convert",
                "confidence": min(85, int(60 + (h_xg + a_xg) * 8)),
            })

    # ── 4. COMEBACK / HT-FT VALUE ──
    if minute >= 40 and minute <= 50:
        # Team losing at HT but dominated xG
        if home_score < away_score and h_xg > a_xg + 0.4:
            alerts.append({
                ...
                "score": f"{home_score}-{away_score}",
                "minute": minute, "league": league,
                "reason": f"{home} trailing but leads xG {h_xg:.1f} vs {a_xg:.1f} — 2nd half comeback likely",
                "confidence": 72,
            })
        elif away_score < home_score and a_xg > h_xg + 0.4:
            alerts.append({
                "market": "HT/FT Value — Comeback",
                "team": away,
                "fixture_id": fixture_id,
                ...
                "score": f"{home_score}-{away_score}",
                "minute": minute, "league": league,
                "reason": f"{away} trailing but leads xG {a_xg:.1f} vs {h_xg:.1f} — 2nd half comeback likely",
                "confidence": 72,
            })
...
def build_market_telegram_message(alert: dict) -> str:
    ...
    line1 = icon + " <b>IN-PLAY ALERT \u2014 PRO</b>"
    return (
        line1 + chr(10) + sep + chr(10)
        + f"\u26bd <b>{home} vs {away}</b>" + chr(10)
        + f"\U0001f3c6 {league}" + chr(10)
        + f"\U0001f550 {minute}' | Score: <b>{score}</b>" + chr(10)
        + sep + chr(10)
        + f"\U0001f4b0 <b>{market}{team_str}</b>" + chr(10)
        + f"\U0001f4ca Confidence: <b>{conf}%</b>" + chr(10) + chr(10)
        + f"\U0001f4cb Why: {reason}" + chr(10)
        + sep + chr(10)
        + "\u26a0\ufe0f <i>Bet responsibly. 18+ only.</i>" + chr(10)
        + "\U0001f916 AccaGenius Pro | In-Play Intelligence"
    )
    ...
    while True:
        try:
            ...
            for m in live_matches:
                try:
                    mid = m["id"]
                    market_cache_key = f"markets_{mid}"
                    if cache_get(market_cache_key):
                        continue  # already alerted this match this cycle
                    ...
                    for ma in market_alerts:
                        if has_pro:
                            ...
                            send_telegram_to(msg, pro_channel)
                            logger.info(f"Market alert: {ma['market']} — {m['home']} vs {m['away']}")

                    for ma in market_alerts:
                        mkey = f"mkt_{mid}_{ma['market'].replace(' ','_')}"
                        if mkey not in alerted_details:
                    ...
                    if market_alerts:
                        cache_set(market_cache_key, True, 300)  # don't re-alert same match for 5min
                except Exception as e:
...
async def admin_gift_pro(request: dict):
    ...
    if not ADMIN_PASSWORD_ENV:
        return {"ok": False, "error": "ADMIN_PASSWORD not set"}
    # Accept raw password OR sha256 hash (same as other admin endpoints)
    expected_hash = hashlib.sha256((ADMIN_PASSWORD_ENV + datetime.now().strftime("%Y-%m-%d")).encode()).hexdigest()[:16]
    if token != ADMIN_PASSWORD_ENV and token != expected_hash:
