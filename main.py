...
async def generate_fbd_value_acca(request: AccaRequest):
    ...
    try:
        ...
        for fbd in fbd_data:
            ...
            ph         = fbd.get("pred_home_odds")
            pd_        = fbd.get("pred_draw_odds")
            pa         = fbd.get("pred_away_odds")
            ah         = fbd.get("avg_home_odds")
            aa         = fbd.get("avg_away_odds")
            btts_prob  = fbd.get("pred_btts")
            ov25_prob  = fbd.get("pred_over25")
            pred_goals = fbd.get("pred_goals")
            ...
            if pred == "H":
                bet = f"{home} Win"
                odds = ah if ah else (1/ph if ph else 1.9)
                if ph and ah and ph > ah * 1.08:
                    value_score += 15
                    reason_parts.append(f"FBD value: pred {ph:.2f} vs market {ah:.2f}")
                value_score += 10  # FBD predicts home
                reason_parts.append(f"FBD predicts HOME win")
            elif pred == "A":
                bet = f"{away} Win"
                odds = aa if aa else (1/pa if pa else 1.9)
                if pa and aa and pa > aa * 1.08:
                    value_score += 15
                ...
                bet = "Draw"
                odds = fbd.get("avg_draw_odds") or 3.2
                value_score += 6
