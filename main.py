...
def score_inplay_markets(fixture_id: int, home: str, away: str,
    ...
    # Only fire 75+ min, game must be a draw, one side clearly dominant
    if is_draw and minute >= 75:
        winner = None
        dom_xg  = 0.0
        dom_poss = 0.0
        dom_shots = 0

        if (h_xg > a_xg + 0.6        # clear xG edge
            and h_poss >= 55          # possession dominance
            and h_shots >= a_shots    # creating more danger
        ):
            winner    = home
            dom_xg    = h_xg
            dom_poss  = h_poss
    ...
    if minute < 45:
        # First half: both teams combining for 2.5+ xG → Over 2.5 at HT
    ...
    if minute < 75 and (home_score == 0 or away_score == 0):
...
async def live_alert_scanner():
    ...
    while True:
        ...
        # Sleep 60s between every scan cycle
