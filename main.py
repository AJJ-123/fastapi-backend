...
def score_inplay_markets(fixture_id: int, home: str, away: str,
    ...
    """
    4 alert types — strict rules:
    1. Match Winner: LEVEL score, 2nd half (45+), dominant xG team
    2. HT/FT Comeback: losing by 1 only, 40-52 min, losing team dominates xG
    3. BTTS: one team scoreless, 30-75 min, both teams 1.0+ xG
    4. Over 1.5 Goals: 0-0 or 1-0/0-1 only, before 65 min, high combined xG
    """
    alerts = []

    # Never alert in first 20 mins
    if minute < 20:
        return []
    ...
    # ── 1. MATCH WINNER ───────────────────────────────────────────────────
    # Scores MUST be level, 2nd half only, strong xG dominance
    ...
    # ── 2. HT/FT COMEBACK ────────────────────────────────────────────────
    # Losing by exactly 1, around HT only, losing team clearly dominates xG
    ...
    # ── 4. OVER 1.5 GOALS ────────────────────────────────────────────────
    # Before 65 min, low score (0-0 or 1-0), high combined xG
    if minute < 65 and total_goals <= 1:
        if total_xg >= 2.5:
...
def _place_goal_due_drip(fixture_id: int, match: str, league: str,
    ...
    Track 1: Simple back of current Over X.5 market
    Track 2: Drip ladder on Over 2.5 with cover bet
    """
    ...
    # Always one market above current score
    # 0 goals → Over 0.5 (simple) / Over 1.5 (drip target)
    # 1 goal  → Over 1.5 (simple) / Over 2.5 (drip target)
    # 2 goals → Over 2.5 (simple) / Over 3.5 (drip target)
    simple_market = f"Over {total_goals + 0.5} Goals"   # current +0.5 for Track A
    drip_market   = f"Over {total_goals + 1.5} Goals"   # one above for Track B drip
    simple_odds   = max(1.3, round(2.5 - (xg_unscored * 0.3), 2))
    ...
    _paper_bot_place_bet(
        ...
        odds=simple_odds,
        stake=_paper_bot_config["stake"],
        reason=f"[TRACK A] Simple back: xG unscored {xg_unscored:.2f} — backing {simple_market}",
        bet_type="back"
    )

    # ── Track B: Drip ladder ──────────────────────────────────────
    if fid in _drip_ladders:
        return  # Already started drip for this fixture

    drip_stakes = drip_cfg.get("stakes", [2.0, 3.0, 5.0])
    drip_drifts = drip_cfg.get("odds_drift", [0.0, 1.0, 2.0])
    # Entry odds scale by how many goals needed — more goals needed = higher odds
    # 0 goals → Over 1.5 entry ~2.0 | 1 goal → Over 2.5 entry ~3.0 | 2 goals → Over 3.5 entry ~5.0
    entry_odds = max(2.0, round(2.0 + (total_goals * 1.0), 2))
    ...
    # Place all drip entries immediately (in paper mode — in live mode these would be queued)
    for i, (stake, drift) in enumerate(zip(drip_stakes, drip_drifts)):
        drip_odds = round(entry_odds + drift, 2)
        label = "Entry" if i == 0 else f"Drip {i}"
        t = _paper_bot_place_bet(
            alert_type="Goal Due — Track B",
            match=match, league=league, minute=minute, score=score,
            selection=f"{drip_market} [{label} @ {drip_odds}]",
            odds=drip_odds, stake=stake,
            reason=f"Ladder {label}: xG unscored {xg_unscored:.2f}, drift +{drift}",
            bet_type="back"
        )
        if t:
            ladder["trade_ids"].append(t["id"])
            ladder["drips_placed"] += 1
            ladder["total_staked"] += stake
            ladder["total_potential_return"] += round((drip_odds - 1) * stake, 2)

    _drip_ladders[fid] = ladder
    _paper_bot_log.insert(0, {
        "time": __import__('datetime').datetime.now().isoformat(),
        "msg": f"🪜 Drip ladder started: {match} — {ladder['drips_placed']} bets, £{ladder['total_staked']:.2f} total staked"
    })
...
def _check_drip_cashout_or_cover(fixture_id: int, match: str, league: str,
    ...
    elif xg_ucover_threshold and not ladder.get("covered"):
...
async def paper_bot_get_trades():
    ...
    won  = [t for t in resolved if t["result"] == "won"]
    lost = [t for t in resolved if t["result"] == "lost"]
    ...
    def track_stats(prefix):
        ...
        tw = [t for t in tr if t["result"] == "won"]
        tl = [t for t in tr if t["result"] == "lost"]
        return {
