...
alerted_details: dict = {}
last_alert_reset: str = ""
_admin_bets:      dict = {}  # goal due / market alert tracking

# =========================
# ══════════════════════════════════════════════════════════════════
# FOOTBALL-BET-DATA.COM — DAILY DOWNLOAD + MATCHING ENGINE
# ══════════════════════════════════════════════════════════════════
import io as _io
...
def _fbd_get_session():
    ...
    vs = _v("__VIEWSTATE"); ev = _v("__EVENTVALIDATION")
    ...
    if "signin" in r2.url:
        print(f"FBD login: FAILED — still on signin page. Check FBD_EMAIL/FBD_PASSWORD", flush=True)
    else:
        print(f"FBD login: SUCCESS — redirected to {r2.url}", flush=True)
    return s
...
# ── FBD Team Name Aliases (API-Football → FBD names) ──
FBD_NAME_ALIASES = {
...
def _fbd_names_match(a: str, b: str) -> bool:
    an = _fbd_normalise(a); bn = _fbd_normalise(b)
...
def fbd_boost_confidence(base_conf: int, home: str, away: str, bet: str) -> int:
    ...
    def prob_to_odds(p):
        try: return round(100/float(p),2) if p and float(p)>0 else None
        except: return None
    if bet == "home":
        ph = prob_to_odds(fbd.get("pred_home_odds")); ah = prob_to_odds(fbd.get("avg_home_odds"))
        if ph and ah and ph > ah*1.05: base_conf = min(95, base_conf + 4)
    if bet == "away":
        pa = prob_to_odds(fbd.get("pred_away_odds")); aa = prob_to_odds(fbd.get("avg_away_odds"))
    ...
    while True:
        try:
                    ...
                    try:
                        ...
                        for m in fbd_matches:
                            ...
                            if bp and float(bp) >= 60:
                                btts_picks.append(f"  ⚽ {home} vs {away} — BTTS {int(float(bp))}%")
                            if op and float(op) >= 60:
                                over25_picks.append(f"  📈 {home} vs {away} — Over 2.5 {int(float(op))}%")
                        sep = "━" * 20
                        if btts_picks and has_pro:
                            send_telegram_to(
                                f"🎯 <b>TODAY'S BTTS VALUE PICKS</b>\n{sep}\n" +
                                "\n".join(btts_picks[:5]) + f"\n{sep}\n📊 FBD prediction model\n🤖 AccaGenius Pro",
                                pro_channel)
                        ...
                        if btts_picks or over25_picks:
                            logger.info(f"9am FBD value bets: {len(btts_picks)} BTTS, {len(over25_picks)} Over 2.5")
            ...
            if cached_live:
                ...
                cache_set("live_matches", {"matches": live_matches}, 55)
            ...
            for m in live_matches:
                try:
                    ...
                    # Cache stats per fixture 60s — reduces API calls
                    stats_cache_key = f"live_stats_{mid}"
                    cached_stats = cache_get(stats_cache_key)
                    if cached_stats:
                        team_stats = cached_stats
                    else:
                        ...
                        FMAP = {"Shots on Goal":"shots_on_goal","Total Shots":"total_shots",
                                ...
                                "Dangerous Attacks":"dangerous_attacks","Attacks":"attacks"}
                        for td in stats_raw:
                            tname = td["team"]["name"]
                            ts = {}
                            for s in td.get("statistics", []):
                                ...
                                ts[key] = val if val is not None else 0
                            # xG proxy: if real xG missing, estimate from shots
                            if not ts.get("expected_goals") or ts.get("expected_goals") == 0:
                                sot = ts.get("shots_on_goal",0) or 0
                                tsh = ts.get("total_shots",0) or 0
                                dan = ts.get("dangerous_attacks",0) or 0
                                proxy = round((sot*0.33)+((tsh-sot)*0.05)+(dan*0.01),2)
                                if proxy > 0:
                                    ts["expected_goals"] = proxy
                                    ts["xg_is_proxy"] = True
                            team_stats[tname] = ts
                        cache_set(stats_cache_key, team_stats, 60)
                ...
                # ── GOAL DUE XG SCANNER ──────────────────────────────────────
                try:
                    home_ts = team_stats.get(m["home"], {})
                    away_ts = team_stats.get(m["away"], {})
                    h_xg = float(home_ts.get("expected_goals") or 0)
                    a_xg = float(away_ts.get("expected_goals") or 0)
                    total_xg = round(h_xg + a_xg, 2)
                    total_goals = (m["home_score"] or 0) + (m["away_score"] or 0)
                    xg_unscored = round(total_xg - total_goals, 2)
                    live_min = m.get("minute") or 0
                    is_proxy = home_ts.get("xg_is_proxy") or away_ts.get("xg_is_proxy")

                    h_shots = int(home_ts.get("shots_on_goal") or 0)
                    a_shots = int(away_ts.get("shots_on_goal") or 0)
                    h_corners = int(home_ts.get("corner_kicks") or 0)
                    a_corners = int(away_ts.get("corner_kicks") or 0)
                    h_dan = int(home_ts.get("dangerous_attacks") or 0)
                    a_dan = int(away_ts.get("dangerous_attacks") or 0)

                    # Dominant team
                    if h_xg > a_xg * 1.3: dominant = m["home"]; dom_shots = h_shots; dom_corners = h_corners
                    elif a_xg > h_xg * 1.3: dominant = m["away"]; dom_shots = a_shots; dom_corners = a_corners
                    else: dominant = None; dom_shots = max(h_shots,a_shots); dom_corners = max(h_corners,a_corners)

                    threshold = 1.2 if is_proxy else 1.0
                    if live_min >= 40 and xg_unscored >= threshold and total_goals < 4 and total_xg > 0:
                        score_snap = f"{m.get('home_score',0)}-{m.get('away_score',0)}"
                        gd_key = f"goaldue_{mid}_{score_snap}"
                        if not cache_get(gd_key):
                            score_str = f"{m['home_score']}-{m['away_score']}"
                            xg_label = "Est. xG (proxy)" if is_proxy else "Real xG"
                            dom_line = f"\n  🎯 <b>{dominant}</b> looks most likely" if dominant else ""
                            signals = []
                            if xg_unscored >= 1.0: signals.append(f"xG {total_xg:.2f} vs {total_goals} goals — {xg_unscored:.2f} unscored")
                            if h_corners+a_corners >= 6: signals.append(f"{h_corners+a_corners} corners — set piece pressure")
                            if dominant and dom_shots >= 4: signals.append(f"{dominant} {dom_shots} shots on target")
                            if h_dan+a_dan >= 20: signals.append(f"{h_dan+a_dan} dangerous attacks combined")
                            signals_text = "\n".join(f"  - {s}" for s in signals[:4])

                            gd_msg = (
                                f"🔴 <b>GOAL DUE ALERT — PRO</b> 🔴\n"
                                f"{'━'*22}\n"
                                ...
                                f"{'━'*22}\n"
                                f"📊 <b>{xg_label}</b>\n"
                                f"  {m['home']}: {h_xg:.2f} xG ({h_shots} SOT)\n"
                                f"  {m['away']}: {a_xg:.2f} xG ({a_shots} SOT)\n"
                                f"  <b>Unscored xG: {xg_unscored:.2f}</b>{dom_line}\n"
                                f"{'━'*22}\n"
                                f"📈 <b>Signals:</b>\n{signals_text}\n\n"
                                f"⚡ Goal pressure building — act now\n"
                                f"💰 Consider: Next Goal / Over markets\n"
                                f"⚠️ <i>18+ | BeGambleAware.org</i>\n"
                                f"🤖 AccaGenius Pro"
                            )
                            if has_pro:
                                sent = send_telegram_to(gd_msg, pro_channel)
                                if sent:
                                    cache_set(gd_key, True, 1800)
                                    logger.info(f"✅ Goal Due: {m['home']} vs {m['away']} {live_min}' xG={xg_unscored} proxy={is_proxy}")
                                    _paper_bot_place_bet(
                                        alert_type="Goal Due", match=f"{m['home']} vs {m['away']}",
                                        league=m["league"], minute=live_min,
                                        score=f"{m['home_score']}-{m['away_score']}",
                                        selection="Next Goal", odds=_estimate_odds("Goal Due",72),
                                        stake=_paper_bot_config["stake"], reason=f"xG unscored:{xg_unscored}",
                                    )
                                    _admin_bets[f"bet_{mid}_goaldue"] = {
                                        "id":f"bet_{mid}_goaldue","type":"Goal Due",
                                        "match":f"{m['home']} vs {m['away']}","league":m["league"],
                                        "minute":live_min,"score_at_alert":score_str,
                                        "xg_unscored":xg_unscored,"fired_at":datetime.now().isoformat(),
        ...
        await asyncio.sleep(60)  # scan every 60s — Ultra plan supports this
...
async def startup_event():
    ...
    if FBD_EMAIL and FBD_PASSWORD:
        ...
        print(f"FBD: ❌ FBD_EMAIL or FBD_PASSWORD not set — check Railway vars", flush=True)
...
async def fbd_daily_scheduler():
    """Download FBD data every morning at 08:15 UK time."""
    import pytz
...
async def get_today_fixtures():
    """FBD-first fixture list. FBD is master, API enriches with logos/times/scores/odds."""
    today_str = _london_now().strftime("%Y-%m-%d")
    ...
    # Fetch all API fixtures in parallel
...
async def fbd_value_bets():
    """Today's BTTS and Over 2.5 value picks from FBD data."""
    data = get_fbd_data()
    btts = []; over25 = []
    for m in data:
        home = m.get("home",""); away = m.get("away","")
        ...
        if bp and float(bp) >= 55:
            btts.append({"home":home,"away":away,"league":m.get("league",""),
                "btts_prob":bp,"avg_odds":m.get("avg_btts_yes"),"fbd_prediction":m.get("prediction","")})
        if op and float(op) >= 55:
            over25.append({"home":home,"away":away,"league":m.get("league",""),
                "over25_prob":op,"avg_odds":m.get("avg_over25"),"pred_goals":m.get("pred_goals"),
    ...
    return {"ok":True,"btts":btts[:10],"over25":over25[:10],"fbd_loaded":len(data)>0,"fbd_match_count":len(data)}


@app.get("/fbd/today")
async def fbd_today_data(token: str = ""):
...
async def generate_fbd_value_acca(request: AccaRequest):
    """FBD Value Acca — picks only from FBD matches, using prediction + value odds."""
    try:
        fbd_data = get_fbd_data()
        if not fbd_data:
            return {"message":"FBD data not loaded yet — try again shortly.","total_selections":0,"total_odds":0,"confidence":0,"selections":[]}
        today = _london_now().date(); today_str = today.strftime("%Y-%m-%d")
        picks = []; seen = set()
        ...
        for fbd in fbd_data:
            home = fbd.get("home","").strip(); away = fbd.get("away","").strip()
            if not home or not away: continue
            pred = fbd.get("prediction","").strip().upper()
            if not pred: continue
            ph = prob_to_odds(fbd.get("pred_home_odds")); pa = prob_to_odds(fbd.get("pred_away_odds"))
            ah = prob_to_odds(fbd.get("avg_home_odds")); aa = prob_to_odds(fbd.get("avg_away_odds"))
            btts_prob = fbd.get("pred_btts"); ov25_prob = fbd.get("pred_over25")

            api_m = next((m for m in cached_today.get("matches",[])
                if _fbd_names_match(home,m.get("home","")) and _fbd_names_match(away,m.get("away",""))), None)
            fix_id = api_m["id"] if api_m else abs(hash(f"{home}{away}{today_str}"))%999999
            if fix_id in seen: continue

            value_score = 0; bet = None; odds = None; reason_parts = []
            if pred == "H":
                bet = f"{home} Win"; odds = ah if ah else 1.9
                if ph and ah and ph > ah*1.05: value_score += 15; reason_parts.append(f"FBD value: pred {ph:.2f} vs market {ah:.2f}")
                value_score += 10; reason_parts.append("FBD predicts HOME win")
            elif pred == "A":
                bet = f"{away} Win"; odds = aa if aa else 1.9
                if pa and aa and pa > aa*1.05: value_score += 15; reason_parts.append(f"FBD value: pred {pa:.2f} vs market {aa:.2f}")
                value_score += 10; reason_parts.append("FBD predicts AWAY win")
            elif pred == "D":
                bet = "Draw"; odds = prob_to_odds(fbd.get("avg_draw_odds")) or 3.2
                value_score += 6; reason_parts.append("FBD predicts DRAW")
            if btts_prob and float(btts_prob) >= 60: value_score += 5; reason_parts.append(f"BTTS {btts_prob:.0f}%")
            if ov25_prob and float(ov25_prob) >= 60: value_score += 5; reason_parts.append(f"Over 2.5 {ov25_prob:.0f}%")
            if not bet or value_score < 10: continue
            if odds: odds = max(1.30, min(float(odds), 6.0))
            else: odds = 1.90
            ...
            picks.append({
                ...
                "date":today_str,"time":api_m.get("time","TBC") if api_m else "TBC",
                "league":api_m.get("league",fbd.get("league","")) if api_m else fbd.get("league",""),
                "league_id":api_m.get("league_id",0) if api_m else 0,
                "bet":bet,"market_type":"1X2","odds":round(odds,2),
                "confidence":min(88,52+value_score),"reasoning":" · ".join(reason_parts),
                "fbd_prediction":pred,"fbd_btts":btts_prob,"fbd_over25":ov25_prob,"fbd_source":True,
            })
            seen.add(fix_id)
        ...
        if not picks:
            return {"message":"No FBD value picks found today.","total_selections":0,"total_odds":0,"confidence":0,"selections":[]}
        total_odds = 1.0
        for p in picks: total_odds *= p.get("odds",1.9)
        avg_conf = sum(p["confidence"] for p in picks) / len(picks)
        return {"message":"FBD Value Acca","total_selections":len(picks),"total_odds":round(total_odds,2),
                "confidence":round(avg_conf),"market":"fbd_value","selections":picks}
    except Exception as e:
...
async def get_today_acca():
    """Generate today's 3 themed accas — FBD-powered, cached all day."""
    today_str = _london_now().strftime("%Y-%m-%d")
    ...
    async def fetch_picks(risk: str, endpoint: str = "form") -> list:
        try:
            if endpoint == "rank":
                ...
                req = AccaRequest(selections=10, risk=risk, leagues=[], market="winner", today_only=True)
                result = await generate_fbd_value_acca(req)
            else:
                req = AccaRequest(selections=8, risk=risk, leagues=leagues, market="winner", today_only=True)
    ...
    fbd_picks  = await fetch_picks("balanced", "fbd")

    # Filter form/rank to FBD-covered matches only
    fbd_data = get_fbd_data()
    def is_fbd_match(p):
        return any(_fbd_names_match(p.get("home",""), m["home"]) and
                   _fbd_names_match(p.get("away",""), m["away"]) for m in fbd_data) if fbd_data else True
    form_picks = [p for p in form_picks if is_fbd_match(p)]
    rank_picks = [p for p in rank_picks if is_fbd_match(p)]

    used_ids = set(); used_teams = set()
    def dedup(picks, n):
        out = []
        for p in picks:
            fid = p.get("id")
            tk = "|".join(sorted([p.get("home",""), p.get("away","")]))
            if fid in used_ids or tk in used_teams: continue
            out.append(p); used_ids.add(fid); used_teams.add(tk)
            if len(out) >= n: break
        return out

    t1 = dedup(form_picks, 3); t2 = dedup(rank_picks, 3); t3 = dedup(fbd_picks, 4)
    result = {
        "date": today_str,
        "accas": [
...
async def generate_acca(request: AccaRequest):
    try:
        # Use FBD league codes if data loaded — faster + better quality
        fbd_data = get_fbd_data()
        FBD_TO_INTERNAL = {
        ...
        if fbd_data and not request.leagues:
            fbd_leagues = list(dict.fromkeys(m.get("league","") for m in fbd_data if m.get("league","")))
            leagues = list(set(FBD_TO_INTERNAL.get(lc,lc) for lc in fbd_leagues if FBD_TO_INTERNAL.get(lc,lc) in LEAGUE_IDS))
            if not leagues:
                leagues = ["PL","ELC","PD","BL1","SA","FL1","NED","TUR","PPL","BEL","SPFL"]
        else:
            ...
            try:
                ...
                for f in fixtures:
                    ...
                    if fbd_today and not any(
                        _fbd_names_match(hn, m["home"]) and _fbd_names_match(an, m["away"])
                        for m in fbd_today
                    ): continue
                    hf = get_team_form(f["teams"]["home"]["id"], lid)
                    af = get_team_form(f["teams"]["away"]["id"], lid)
                    if hf.get("games",0) == 0 and af.get("games",0) == 0: continue
                    pick = analyze_and_pick(f, hf, af, request.risk, request.market)
                    # FBD confidence boost
                    if pick:
                        bet_lower = pick.get("bet","").lower()
                        side = "draw" if "draw" in bet_lower else ("away" if an.lower() in bet_lower else "home")
                        pick["confidence"] = fbd_boost_confidence(pick["confidence"], hn, an, side)
                        fbd_m = get_fbd_match(hn, an)
                        if fbd_m:
                            pick["fbd_prediction"] = fbd_m.get("prediction","")
                            pick["fbd_btts"]       = fbd_m.get("pred_btts")
                            pick["fbd_over25"]     = fbd_m.get("pred_over25")
                    if pick and pick["confidence"] >= 45:
                        picks.append(pick); picks_this_league += 1
            except Exception as e:
        ...
        avg_conf = sum(p["confidence"] for p in picks) / len(picks)
        return {
            "message": f"AI Acca — {request.risk.capitalize()} risk — FBD picks",
            "total_selections": len(picks), "total_odds": round(total_odds,2),
            "confidence": round(avg_conf), "risk_level": request.risk,
            "market": request.market, "selections": picks
        }
    except Exception as e:
        raise HTTPException(500, str(e))
        today = _london_now().date()
...
async def generate_ht_acca(request: HTAccaRequest):
    try:
        ...
        for lc in leagues:
            ...
            for f in fixtures:
                ...
                if pick and pick["confidence"] >= 60:
                    seen.add(fid)
                    odds = get_real_odds(fid)
                    pick["odds_home"] = odds.get("home",0); pick["odds_draw"] = odds.get("draw",0)
                    pick["odds_away"] = odds.get("away",0); pick["odds_home_bk"] = odds.get("home_bk","")
        ...
        if not picks:
            return {"message":"No HT picks found","total_selections":0,"total_odds":0,"confidence":0,"selections":[]}
        total_odds = 1.0
        for p in picks: total_odds *= p["odds"]
        avg_conf = sum(p["confidence"] for p in picks) / len(picks)
        return {"message":"HT Acca","total_selections":len(picks),"total_odds":round(total_odds,2),"confidence":round(avg_conf),"selections":picks}
    except Exception as e:
        raise HTTPException(500, str(e))
...
async def generate_btts_acca(request: AccaRequest):
    """BTTS acca — FBD predicted probability first, API form fallback."""
    try:
        today = _london_now().date()
        ...
        if fbd_data:
            ...
            for fbd in fbd_data:
                ...
                if not home or not away or not btts_prob or float(btts_prob) < 55: continue
                api_m = next((m for m in cached_today.get("matches",[])
                    if _fbd_names_match(home,m.get("home","")) and _fbd_names_match(away,m.get("away",""))), None)
                ...
                all_picks.append({
                    ...
                    "odds_available":odds.get("available",False),"confidence":conf,
                    "fbd_btts":btts_prob,"fbd_source":True,
                    "reasoning":f"FBD BTTS {btts_prob:.0f}% · Pred: {fbd.get('prediction','')}",
                })
                seen.add(fix_id)
        else:
            # Fallback: API form-based
            leagues = request.leagues or ["PL","ELC","BL1","SA","FL1","PD"]
            cutoff = today + timedelta(days=3)
            def fetch_btts(lc):
                lid = LEAGUE_IDS.get(lc)
                if not lid: return []
                ...
                try:
                    ...
                    for f in sorted(data.get("response",[]), key=lambda x: x["fixture"]["date"]):
                        ...
                        fd = datetime.fromisoformat(f["fixture"]["date"].replace("Z","+00:00")).date()
                        if fd < today or fd > cutoff: continue
                        if f["fixture"]["status"]["short"] in ("1H","HT","2H","ET","P","FT","AET","PEN"): continue
                        hf = get_team_form(f["teams"]["home"]["id"], lid)
                        af = get_team_form(f["teams"]["away"]["id"], lid)
                        cb = round((hf.get("btts_pct",0)+af.get("btts_pct",0))/2)
                        if cb >= 55:
                            dt = datetime.fromisoformat(f["fixture"]["date"].replace("Z","+00:00"))
                            odds = get_real_odds(fid)
                            picks.append({"id":fid,"home":f["teams"]["home"]["name"],"away":f["teams"]["away"]["name"],
                                ...
                                "bet":"Both Teams to Score","market_type":"BTTS",
                                "odds":odds.get("btts",1.72),"confidence":min(88,55+int(cb*0.4)),
                                "reasoning":f"Combined BTTS% {cb}%"})
                except: pass
                return picks
            with ThreadPoolExecutor(max_workers=10) as ex:
                loop = asyncio.get_event_loop()
                for lp in await asyncio.gather(*[loop.run_in_executor(ex, fetch_btts, lc) for lc in leagues]):
                    for p in lp:
                        if p["id"] not in seen: seen.add(p["id"]); all_picks.append(p)
...
async def delete_saved_acca(request: dict):
    ...
    global saved_accas_store
    saved_accas_store = [a for a in saved_accas_store if str(a.get("id","")) != acca_id]
    try:
...
async def reset_saved_accas(request: dict):
    """Permanently clear all saved accas for a user from DB and memory."""
    email = (request.get("email") or "").strip().lower()
    if not email:
