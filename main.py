...
async def _trading_scheduler():
    ...
    while True:
        try:
            ...
            # 7:15 — 7am price capture fallback
            if _due(tasks["prices_7am"] - timedelta(days=1)):
                logger.info("💰 7:15am — 7am price capture fallback...")
                try:
                    capture_exchange_prices("7am")
                    logger.info("✅ 7am prices captured")
                except Exception as e:
                    logger.error(f"7am capture error: {e}")
            ...
            # Pre-KO price capture every 5 mins between 12pm and 10pm
            if 12 <= now.hour < 22 and now.minute % 5 == 0:
                try:
                    capture_exchange_prices("5m")
                except Exception as e:
                    logger.error(f"Pre-KO capture error: {e}")
