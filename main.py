...
def _generate_prematach_sheet() -> bytes:
    """
    Build the pre-match Excel sheet with:
    - Today's fixtures from FBD
    - Live Betfair exchange prices from PC capture
    - Poisson model odds
    - Value columns
    ...
    try:
        ...
        BACK_COL="FF9DC3E6"; LAY_COL="FFFFC7CE"; PL_COL_H="FFFFFF00"
        ...
        HDRS=[
            ...
            "H Rank","A Rank","H Colour","A Colour",
            "H GP","H W","H D","H L","H Pts",
            "A GP","A W","A D","A L","A Pts",
            "Score 1","Score 2","Score 3",
            "HxG 45","AxG 45","Match xG","H Sup",
            ...
            # Betfair prices — back only, no PL on pre-match
        ...
        for i in range(6,10):  H[i]=(SEC_MATCH,"FFAAB4D4")
        for i in range(10,20): H[i]=(SEC_FORM,AG_GOLD)
        for i in range(20,23): H[i]=(SEC_SCORE,AG_GOLD)
        for i in range(23,27): H[i]=(SEC_POIS_N,WHITE)
        for i in range(27,35): H[i]=(SEC_POIS_O,"FFDDBBFF")
        for i in range(35,43): H[i]=(SEC_VAL,AG_GOLD)
        ...
        SEC_NAMES=[
            (1,6,"MATCH DETAILS"),(7,10,"LEAGUE & FORM"),
            (11,20,"FORM  HOME / AWAY"),(21,23,"TOP 3 SCORELINES"),
            (24,35,"POISSON MODEL"),(36,43,"VALUE  —  Model vs Market"),
            (44,50,"MATCH ODDS"),(51,55,"UNDER / OVER 2.5"),
            (56,60,"UNDER / OVER 1.5"),(61,65,"UNDER / OVER 3.5"),
            (66,70,"UNDER / OVER 4.5"),(71,75,"BTTS"),
            (76,80,"FIRST HALF  U/O 0.5"),(81,N,"FIRST HALF  U/O 1.5"),
        ]
        ...
        for fbd in fbd_data:
            ...
            if pred>0 and (h_prob+a_prob)>0:
                ...
                home_xg = round(pred*h_share,2)
                away_xg = round(pred*(1-h_share),2)
            else:
            ...
            # Get live Betfair prices from PC capture
            steam  = get_steam_for_fixture(home, away)
            mkts   = steam.get("markets",{}) if steam else {}

            def gp(mkt,side="7am"):
                """Get price from steam store."""
                m=mkts.get(mkt,{})
                return m.get(side) or m.get("5m") or None

            # Value calculation vs Betfair prices
            o6 = poisson.get("odds_6",{})
            p6 = poisson.get("prob_6",{})
            def val(prob, price):
            ...
            fixtures.append({
                ...
                # Betfair prices
                "home_back": gp("home_back"),
                "home_lay":  gp("home_lay"),
                "away_back": gp("away_back"),
                "away_lay":  gp("away_lay"),
                "draw_back": gp("draw_back"),
                "draw_lay":  gp("draw_lay"),
                "o25_back":  gp("o25_back"),
                "o15_back":  gp("o15_back"),
                "u25_back":  gp("u25_back"),
                "u15_back":  gp("u15_back"),
                "fho_back":  gp("fh_o05_back"),
                # Value
                "val_home": val(p6.get("home_win",0), gp("home_back")),
                "val_draw": val(p6.get("draw",0),     gp("draw_back")),
                "val_away": val(p6.get("away_win",0), gp("away_back")),
                "val_o25":  val(p6.get("over_25",0),  gp("o25_back")),
                "val_o15":  val(p6.get("over_15",0),  gp("o15_back")),
            })
        ...
        ws.cell(row=1,column=1).value="AGD Sports Trading"
        ...
        ws.cell(row=1,column=8).value=(
            ...
            f"Betfair prices captured at 7am  |  "
            f"Blue=Back  Pink=Lay  |  18+ Gamble Responsibly")
        ...
        for sc,ec,name in SEC_NAMES:
            ...
            c=ws.cell(row=2,column=sc); c.value=name
        ...
        for i,hdr in enumerate(HDRS,1):
            ...
            c.font=Font(name="Calibri",size=8,bold=True,color=fc)
            c.fill=fill(bg); c.alignment=ctr(True); c.border=bdr()
        ...
        WID={0:13,1:8,2:20,3:16,4:16,5:5}
        for i in range(N):
            ws.column_dimensions[get_column_letter(i+1)].width=WID.get(i,8)

        # ── DATA ROWS ────────────────────────────────────────────
        CI_MAP={"Green":"FF00B050","Amber":"FFFFC000","Red":"FFFF0000","Grey":"FF808080"}

        for ri,fix in enumerate(fixtures,5):
            ws.row_dimensions[ri].height=14
            p6=fix["poisson"].get("odds_6",{})
            ps=fix["poisson"].get("odds_s",{})
            v6=fix["poisson"].get("val_6",{})
            vs=fix["poisson"].get("val_s",{})
            top3=fix.get("top3",["","",""])
            while len(top3)<3: top3.append("")

            row_vals=[
                today.strftime("%d/%m/%Y"),
                "",  # KO time — FBD doesn't always have it
                fix.get("league",""),
                fix["home"], fix["away"], "",
                "","",  # H/A rank
                "","",  # colour index
                ...
                round(fix["home_xg"]-fix["away_xg"],2),
                p6.get("home",""),p6.get("draw",""),p6.get("away",""),
                p6.get("over_15",""),p6.get("over_25",""),
                p6.get("over_35",""),p6.get("btts",""),p6.get("fh_over_05",""),
                fix.get("val_home",""),fix.get("val_draw",""),fix.get("val_away",""),
                fix.get("val_o25",""),fix.get("val_o25",""),
                "","","","",  # val o35 btts fh
                # Betfair prices
                "",  # MO vol
                fix.get("home_back",""),fix.get("home_lay",""),
                fix.get("away_back",""),fix.get("away_lay",""),
                fix.get("draw_back",""),fix.get("draw_lay",""),
                "",  # 2.5 vol
                fix.get("u25_back",""),"",
                fix.get("o25_back",""),"",
                "",  # 1.5 vol
            ...
            for ci,val in enumerate(row_vals[:N],1):
                ...
                if ci in(4,5):
                    c.font=Font(name="Calibri",size=8,bold=True,color=AG_GOLD)
                    c.alignment=lft()
                elif ci==1:
                    c.font=Font(name="Calibri",size=8,color=WHITE); c.alignment=lft()
                elif ci in(2,3):
                    c.font=Font(name="Calibri",size=8,color=MUTED); c.alignment=lft()
                elif ci in range(36,44):  # value cols
                    try:
                        v=float(sv)
                        c.font=Font(name="Calibri",size=8,bold=True,
                                   color=PL_POS if v>0 else(PL_NEG if v<0 else WHITE))
                    except: c.font=Font(name="Calibri",size=8,color=WHITE)
                    c.alignment=ctr()
                else:
                    c.font=Font(name="Calibri",size=8,color=WHITE)
                    c.alignment=ctr() if ci>6 else lft()
...
async def download_prematch_sheet():
    """
    Generate and download today's pre-match Excel sheet.
    Contains live Betfair prices + Poisson model + value columns.
    Call after 7am when prices have been captured.
    """
    try:
        today    = datetime.now().strftime("%Y-%m-%d")
        filename = f"AGD_PreMatch_{today}.xlsx"
        ...
        logger.error(f"/sheet/prematch error: {e}")
        return {"error": str(e)}

@app.get("/sheet/steam")
async def download_steam_sheet():
    """
    Generate and download today's steam detection sheet.
    Shows all fixtures with 7am vs 5min pre-KO price movements.
    Only useful after both captures have run.
    """
    try:
        from openpyxl import Workbook
        from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
        ...
        today  = datetime.now()
        HDRS=["Fixture","League","7am O2.5","5m O2.5","O2.5 Move","O2.5 %","O2.5 Steam",
        ...
        ws.cell(row=1,column=1).font=Font(name="Calibri",size=11,bold=True,color=AG_GOLD)
        ws.cell(row=1,column=1).fill=fill(AG_BLACK); ws.cell(row=1,column=1).alignment=lft()
        for ci in range(2,N+1): ws.cell(row=1,column=ci).fill=fill(AG_BLACK)

        ws.row_dimensions[2].height=14
        ws.cell(row=2,column=1).value="System 1 LTD: Draw+O2.5 steam  ·  System 2 LU1.5: O2.5 steams  ·  System 3 LFHU0.5: O1.5 steams  ·  Threshold: 3%"
        ...
        hdr_bgs={"Fixture":(AG_DARK,AG_GOLD),"League":(AG_DARK,MUTED),
                 "LTD?":("FF1A0000","FFFF6666"),"LU1.5?":("FF001A00","FF66FF66"),
                 "LFHU0.5?":("FF001A1A","FF66FFFF")}
        for i,hdr in enumerate(HDRS,1):
            c=ws.cell(row=3,column=i,value=hdr)
            if "7am" in hdr: bg,fc=(("FF003366","FFAACCFF"))
            elif "5m" in hdr: bg,fc=(("FF006633","FFAAFFCC"))
            elif "Move" in hdr: bg,fc=(("FF332600",AG_GOLD))
            elif "%" in hdr: bg,fc=(("FF332600",AG_GOLD))
            elif "Steam" in hdr: bg,fc=((STEAM_HDR,BLACK))
            else: bg,fc=hdr_bgs.get(hdr,(AG_DARK,WHITE))
            c.font=Font(name="Calibri",size=8,bold=True,color=fc)
            c.fill=fill(bg); c.alignment=ctr(); c.border=bdr()
        ...
        for fkey,fdata in _steam_prices.items():
            ...
            def mv(p7,p5):
                ...
                m=round(p5-p7,2); pct=round(abs(m)/p7*100,1)
                return m,pct
            def sf(m,pct):
            ...
            o25_m,o25_p=mv(o25_7,o25_5)
            draw_m,draw_p=mv(draw_7,draw_5)
            fho_m,fho_p=mv(fho_7,fho_5)
            o15_m,o15_p=mv(o15_7,o15_5)

            o25_s=sf(o25_m,o25_p); draw_s=sf(draw_m,draw_p)
            fho_s=sf(fho_m,fho_p); o15_s=sf(o15_m,o15_p)

            o25_steam=o25_s=="STEAM"; draw_steam=draw_s=="STEAM"
            o15_steam=o15_s=="STEAM"

            sys1="✅ LTD" if (draw_steam and o25_steam) else ""
            sys2="✅ LU1.5" if o25_steam else ""
            sys3="✅ LFHU0.5" if o15_steam else ""

            has_steam=any([o25_steam,draw_steam,o15_steam,fho_s=="STEAM"])
            ri=rows_written+4

            row_vals=[
                f"{home} v {away}",league,
            ...
            for ci,val in enumerate(row_vals[:N],1):
                c=ws.cell(row=ri,column=ci,value=val); sv=str(val) if val else ""
                c.fill=fill(ALERT_BG if has_steam else(ALT if ri%2==0 else MID))
                if ci==1:
                    c.font=Font(name="Calibri",size=8,bold=True,color=AG_GOLD if has_steam else WHITE)
                    c.alignment=lft()
                elif ci==2:
                    c.font=Font(name="Calibri",size=8,color=MUTED); c.alignment=lft()
                elif "Steam" in HDRS[ci-1] or "%" in HDRS[ci-1]:
                    if "STEAM" in sv: col="FFFF6600"
                    elif "DRIFT" in sv: col=GREEN
                    else:
                        try: col="FFFF6600" if float(sv.replace("%",""))>=THRESH else MUTED
                        except: col=MUTED
                    c.font=Font(name="Calibri",size=8,bold=("STEAM" in sv or "DRIFT" in sv),color=col)
                    c.alignment=ctr()
                    ...
                    c.alignment=ctr()
                elif HDRS[ci-1] in("LTD?","LU1.5?","LFHU0.5?"):
                    c.font=Font(name="Calibri",size=8,bold=bool(sv),
                               color=GREEN if sv else MUTED)
                    c.alignment=ctr()
                else:
        ...
        return StreamingResponse(
            buf,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": f"attachment; filename=AccaGenius_Steam_{today_str}.xlsx"}
        )
    except Exception as e:
        logger.error(f"/sheet/steam error: {e}")
        return {"error": str(e)}

logger.info("✅ Sheet download endpoints loaded — /sheet/prematch /sheet/steam")


# ═══════════════════════════════════════════════════════════════════
# TRADING PLAN — £1 trial / £300 per year
# Separate from £20/month Pro plan
# ═══════════════════════════════════════════════════════════════════

TRADING_SEASONS = ["2020-21","2021-22","2022-23","2023-24","2024-25","2025-26"]
SEASON_FILES_PATH = os.getenv("SEASON_FILES_PATH", "/app/season_files")
...
def _is_trading(email: str) -> bool:
    """Check if user has trading plan access."""
    if _is_admin(email): return True
    user = _user_db.get(email.lower(), {})
...
@app.post("/create-trading-checkout")
async def create_trading_checkout(request: dict):
    """
    Create Stripe checkout for Trading Plan.
    £1 for first 30 days, then £300/year.
    """
    ...
    # Check if already has trading plan
    user = _user_db.get(email, {})
    if user.get("plan") == "trading":
        return {"error": "Already on trading plan"}

    # Check if already used trial
    if user.get("had_trading_trial") and user.get("plan") == "trading_trial":
        return {"error": "Trial already used — please subscribe at full price"}

    try:
        # Build line items — £1 trial then £300/year
        if STRIPE_TRADING_PRICE_ID:
            # Use pre-created Stripe price
            line_items = [{"price": STRIPE_TRADING_PRICE_ID, "quantity": 1}]
            sub_data   = {"trial_period_days": 30}
        else:
            # Inline price — £150/year (early bird 50% off £300)
            # First 30 days free trial then £150/year
            line_items = [{
                "price_data": {
                    "currency":     "gbp",
                    "unit_amount":  15000,  # £150 early bird
                    "recurring":    {"interval": "year"},
                    "product_data": {
                        "name":        "AGD Sports Trading Plan — Early Bird",
                        "description": "Daily pre-match sheets · 6 seasons Betfair data · Poisson model · Steam detection · 50% early bird — was £300",
                    },
                },
                "quantity": 1,
            }]
            sub_data = {
                ...
                "metadata": {"email": email, "plan": "trading"},
            }
        ...
        session = _stripe.checkout.Session.create(
            ...
            success_url=f"{SITE_URL}/trading?success=1&email={email}",
            cancel_url=f"{SITE_URL}/trading?cancelled=1",
            metadata={"email": email, "name": name, "plan": "trading"},
        )

        # Mark trial as started
        if email not in _user_db:
            _user_db[email] = {"email": email, "name": name, "plan": "free",
                               "plan_status": "free", "role": "user"}
        _user_db[email]["had_trading_trial"] = True
        _save_user_db()

        return {"url": session.url}

    except Exception as e:
        logger.error(f"Trading checkout error: {e}")
        return {"error": str(e)}

@app.get("/trading/access")
async def check_trading_access(email: str):
    """Check if a user has trading plan access."""
    if not email:
        return {"access": False}
    has_access = _is_trading(email)
...
# ── SEASON FILE DOWNLOADS ────────────────────────────────────────
@app.get("/sheet/season/{season}")
async def download_season_file(season: str, email: str = ""):
    """
    Download a historical season results file.
    Requires trading plan or admin access.
    Season format: 2020-21, 2021-22 etc
    """
    if not _is_trading(email):
        return {"error": "Trading plan required. Upgrade at accagenius.com/trading"}

    if season not in TRADING_SEASONS:
        return {"error": f"Invalid season. Available: {', '.join(TRADING_SEASONS)}"}

    try:
        # Generate the season file on the fly
        content = await _generate_season_file(season)
        filename = f"AGD_{season}_Results.xlsx"
        return StreamingResponse(
            io.BytesIO(content),
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": f"attachment; filename={filename}"}
        ...
        logger.error(f"Season download error {season}: {e}")
        return {"error": str(e)}
...
async def download_steam_sheet_protected(email: str = ""):
    """
    Download personal steam detection sheet.
    ADMIN ONLY — never shared with members.
    """
    if not _is_admin(email):
        return {"error": "Admin access only"}

    try:
        # Generate steam sheet (existing function generates it)
        return await download_steam_sheet_internal()
    except Exception as e:
