...
# ── STRIPE ───────────────────────────────────────────────────────────────────
STRIPE_SECRET_KEY        = os.getenv("STRIPE_SECRET_KEY", "sk_test_51T7zKIC2iUpRnqYDN1GdhxTByYL8jb9TYkOSeBL7BQIiICqqSyt5hfzzy7kt4hYNROCoIh16KBdRm8nfVtLL5znG00SI0OF04Z")
...
async def create_checkout_session(request: dict):
    ...
    if not STRIPE_SECRET_KEY:
        return {"error": "Stripe not configured — set STRIPE_SECRET_KEY in Railway"}
...
async def stripe_webhook(request):
    ...
    if STRIPE_WEBHOOK_SECRET:
        try:
            _verify_stripe_signature(body, sig, STRIPE_WEBHOOK_SECRET)
        except Exception as e:
