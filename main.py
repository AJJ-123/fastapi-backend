...
async def admin_gift_pro(request: dict):
    ...
    check = token or pwd
    logger.info(f"Gift auth: received='{check[:4]}...' expected='{ADMIN_PASSWORD_ENV[:4]}...' match={check==ADMIN_PASSWORD_ENV}")
    if not ADMIN_PASSWORD_ENV or check != ADMIN_PASSWORD_ENV:
        return {"ok": False, "error": f"Unauthorised — token length {len(check)} vs expected {len(ADMIN_PASSWORD_ENV)}"}
