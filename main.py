...
async def admin_gift_pro(request: dict):
    ...
    if email in _user_db:
        # Already exists — upgrade to pro and send email
        existing_name = name or _user_db[email].get("name", "there")
        _user_db[email]["plan"] = "pro"
        _user_db[email]["gifted"] = True
        _user_db[email]["telegram_invite"] = TELEGRAM_PRO_INVITE_LINK
        _save_user_db()
        email_sent = email_welcome_pro(email, existing_name, "GIFTED", TELEGRAM_PRO_INVITE_LINK)
        return {"ok": True, "message": f"{existing_name} ({email}) upgraded to Pro ✅", "already_existed": True, "email_sent": email_sent}
...
async def admin_generate_codes(request: dict):
    ...
    # Save new codes directly to database
    pg = _get_pg()
    saved = []
    try:
        if pg:
            ...
            for code in new_codes:
                if code not in _all_codes:
                    cur.execute("INSERT INTO codes (code, source) VALUES (%s, %s) ON CONFLICT DO NOTHING", (code, "admin_generated"))
                    _all_codes.add(code)
                    saved.append(code)
            pg.commit(); pg.close()
        else:
            ...
            for code in new_codes:
                if code not in _all_codes:
                    conn.execute("INSERT OR IGNORE INTO codes (code, source) VALUES (?, ?)", (code, "admin_generated"))
                    _all_codes.add(code)
                    saved.append(code)
            conn.commit(); conn.close()
    except Exception as e:
        logger.error(f"generate-codes DB error: {e}")

    combined = sorted(set(list(_all_codes)))
    return {"ok": True, "new_codes": saved, "total_codes": len(_all_codes),
            "railway_value": ",".join(combined),
            "message": f"{len(saved)} codes saved to database and ready to use immediately"}
...
async def admin_send_code_email(request: dict):
    ...
    # Add code to _all_codes and DB so it passes validation on signup
    _all_codes.add(code)
    db_save_code(code, "", "manual")
