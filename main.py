...
TELEGRAM_BOT_TOKEN    = os.getenv("TELEGRAM_BOT_TOKEN", "8459345549:AAHMvrNwayhs8zWL_A-RoYrh9TF58OC01Dc")
...
@app.get("/admin/codes")
async def admin_codes(token: str = ""):
    import hashlib
...
async def generate_acca(request: AccaRequest):
    try:
        ...
        # If today_only, pull from the /today cache to avoid duplicate API calls
        _today_cache_key = f"today_{datetime.now().strftime('%Y-%m-%d')}"
        _today_cached = cache_get(_today_cache_key)
        _today_fixtures_by_league: dict = {}
        if _today_cached and request.today_only:
            for m in _today_cached.get("matches", []):
                lc_key = m.get("league_code", "")
                _today_fixtures_by_league.setdefault(lc_key, []).append(m.get("_raw"))
        ...
        def fetch_league_picks(lc: str) -> list:
            ...
            picks = []
            try:
                # Reuse cached /today data if available (saves API calls)
                if request.today_only and lc in _today_fixtures_by_league:
                    raw_fixtures = [r for r in _today_fixtures_by_league[lc] if r]
                    # Fall through to api_get if no raw fixtures cached
                    if not raw_fixtures:
                    ...
                    data = api_get("fixtures", {"league": lid, "season": get_season(),
                                                "next": 10, "timezone": "Europe/London"})
                    raw_fixtures = data.get("response", [])
                fixtures = sorted(raw_fixtures, key=lambda f: f["fixture"]["date"] if isinstance(f, dict) and "fixture" in f else "")
                picks_this_league = 0
                for f in fixtures:
                    if picks_this_league >= 3: break  # max 3 per league = max 6 form calls
                    fid = f["fixture"]["id"]
