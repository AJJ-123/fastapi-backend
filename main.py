    ...
    try:
        ...
        for row in target.iter_rows(values_only=True):
            ...
            matches.append({
                ...
                # These are odds — convert to probabilities
                ...
                # pred_goals is a raw goals number (e.g. 2.6) — NOT odds
...
def _generate_prematach_sheet() -> bytes:
    ...
    try:
        ...
        for fbd in fbd_data:
            ...
            if not home or not away: continue
            pred  = fbd.get("pred_goals") or 0    # raw goals e.g. 2.6
            h_pct = fbd.get("pred_home_odds") or 0  # home win % e.g. 45.2
            a_pct = fbd.get("pred_away_odds") or 0  # away win % e.g. 28.1
            if 0 < pred < 10 and (h_pct+a_pct) > 0:
                h_share = h_pct / (h_pct + a_pct)
                home_xg = round(pred * h_share, 2)
                away_xg = round(pred * (1 - h_share), 2)
            elif 0 < pred < 10:
                home_xg = round(pred * 0.55, 2)
                away_xg = round(pred * 0.45, 2)
            else:
...
async def download_steam_sheet_protected(email: str = ""):
    ...
    try:
        ...
        for fkey,fdata in _steam_prices.items():
            ...
            # Calculate Poisson for this fixture
            poisson_data={}; val_data={}
            try:
                fbd_match=get_fbd_match(home,away)
                if fbd_match:
                    pred=fbd_match.get("pred_goals") or 0
                    h_pct=fbd_match.get("pred_home_odds") or 0
                    a_pct=fbd_match.get("pred_away_odds") or 0
                    if 0<pred<10 and (h_pct+a_pct)>0:
                        hs=h_pct/(h_pct+a_pct)
                        hxg=round(pred*hs,2); axg=round(pred*(1-hs),2)
                    else:
                        hxg=1.35; axg=1.10
                    pois=build_poisson_for_match(hxg,axg,hxg,axg,home,away)
                    o6=pois.get("odds_6",{}); p6=pois.get("prob_6",{})
                    poisson_data={"o25":o6.get("over_25",""),"draw":o6.get("draw",""),
                                  "o15":o6.get("over_15",""),"fh_o05":o6.get("fh_over_05","")}
                    def _sv(prob,price):
                        if not price or price<=1: return ""
                        v=round((prob-1/price)*100,1)
                        return v if abs(v)>0.5 else ""
                    val_data={"o25":_sv(p6.get("over_25",0),o25_7),
                              "draw":_sv(p6.get("draw",0),draw_7),
                              "o15":_sv(p6.get("over_15",0),o15_7),
                              "fh_o05":_sv(p6.get("fh_over_05",0),fho_7)}
            except Exception as _pe:
                logger.warning(f"Poisson calc error in steam sheet: {_pe}")
            ...
            row_vals=[
                ...
                # Poisson — calculated from steam xG data if available
                poisson_data.get("o25",""), poisson_data.get("draw",""),
                poisson_data.get("o15",""), poisson_data.get("fh_o05",""),
                # Value vs 7am price
                val_data.get("o25",""), val_data.get("draw",""),
                val_data.get("o15",""), val_data.get("fh_o05",""),
                sys1, sys2, sys3, "",
            ]
