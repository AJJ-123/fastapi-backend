...
# ═══════════════════════════════════════════════════════════════════
# BETFAIR API — Price Capture
# Free Delay Key — captures 7am and 5min pre-KO prices
# ═══════════════════════════════════════════════════════════════════
...
# ── Session cache ────────────────────────────────────────────────
_bf_session: dict = {"token": None, "expires": None}

def _bf_login() -> str | None:
    """
    Login to Betfair using non-interactive endpoint.
    Works with delay app key — no SSL cert required.
    """
    global _bf_session
    now = datetime.utcnow()
    if _bf_session["token"] and _bf_session["expires"] and now < _bf_session["expires"]:
        return _bf_session["token"]
    if not BETFAIR_APP_KEY or not BETFAIR_USERNAME or not BETFAIR_PASSWORD:
        logger.warning("Betfair credentials not set in Railway environment variables")
        return None
    try:
        # Non-interactive login — works without SSL certificate
        r = _requests.post(
            BETFAIR_LOGIN_URL,
            params={"username": BETFAIR_USERNAME, "password": BETFAIR_PASSWORD},
            headers={
        ...
        logger.info(f"Betfair login response status: {r.status_code}")
        logger.info(f"Betfair login response: {r.text[:200]}")

        if not r.text.strip():
            logger.error("Betfair login returned empty response")
            return None
        ...
        if status == "SUCCESS":
            ...
            _bf_session["token"]   = token
            _bf_session["expires"] = now + timedelta(hours=4)
            logger.info("✅ Betfair login successful")
            return token
        else:
            error = data.get("error") or data.get("Error", "unknown")
            logger.error(f"Betfair login failed: status={status} error={error}")
...
def _bf_api(method: str, params: dict) -> dict:
    ...
    try:
        ...
        logger.error(f"Betfair API error ({method}): {e}")
        return {}

def _bf_get_football_markets(date_str: str = None) -> list:
    """
    Get all football markets for today.
    Returns list of markets with their IDs.
    """
    if not date_str:
        date_str = datetime.utcnow().strftime("%Y-%m-%dT00:00:00Z")
    end_str = (datetime.utcnow() + timedelta(days=1)).strftime("%Y-%m-%dT00:00:00Z")
    try:
        result = _bf_api("listMarketCatalogue", {
            "filter": {
                "eventTypeIds": ["1"],  # 1 = football
                "marketStartTime": {"from": date_str, "to": end_str},
                "marketCountries": ["GB","DE","ES","IT","FR","NL","PT","BE","PL","TR",
                                    "DK","CH","AT","NO","SCO"],
                "marketTypeCodes": ["MATCH_ODDS","OVER_UNDER_25","OVER_UNDER_15",
                                    "OVER_UNDER_35","BOTH_TEAMS_TO_SCORE",
        ...
        logger.error(f"_bf_get_football_markets error: {e}")
        return []

def _bf_get_prices(market_ids: list) -> dict:
    """
    Get best available back and lay prices for a list of market IDs.
    Returns dict of market_id -> runner prices.
    """
    if not market_ids:
        return {}
    try:
        # Betfair limits to 200 markets per call
        chunks = [market_ids[i:i+200] for i in range(0, len(market_ids), 200)]
        all_prices = {}
        for chunk in chunks:
            result = _bf_api("listMarketBook", {
                "marketIds": chunk,
                "priceProjection": {
                    "priceData": ["EX_BEST_OFFERS"],
                    "exBestOfferOverRides": {"bestPricesDepth": 1},
                },
                ...
                for market in result:
                    ...
                    for runner in market.get("runners", []):
                        ...
                        runners[rid] = {
                            ...
                            "back_size": back[0].get("size", 0) if back else 0,
                        }
                    all_prices[mid] = runners
        return all_prices
    except Exception as e:
        logger.error(f"_bf_get_prices error: {e}")
        return {}

# ── Price capture storage ────────────────────────────────────────
# Stored as: fixture_key -> {market -> {7am: price, 5m: price, move: diff}}
_steam_prices: dict = {}
_steam_date:   str  = ""
...
def _parse_bf_market_name(market_name: str) -> str:
    """Normalise market name to our standard labels."""
    mn = market_name.lower()
    if "match odds"      in mn: return "match_odds"
    if "over/under 2.5"  in mn: return "over_25"
    if "over/under 1.5"  in mn: return "over_15"
    if "over/under 3.5"  in mn: return "over_35"
    if "both teams"      in mn: return "btts"
    return mn.replace(" ", "_")

def capture_betfair_prices(capture_type: str = "7am") -> dict:
    """
    Main price capture function.
    capture_type: "7am" or "5m" (5 minutes pre-KO)
    Fetches all football markets and stores best back prices.
    """
    ...
    logger.info(f"📊 Betfair price capture starting — type: {capture_type}")
    markets = _bf_get_football_markets()
    if not markets:
        logger.warning("No Betfair markets found")
        return {}

    market_ids = [m.get("marketId") for m in markets if m.get("marketId")]
    prices     = _bf_get_prices(market_ids)

    captured = 0
    for market in markets:
        mid    = market.get("marketId")
        event  = market.get("event", {})
        mtype  = _parse_bf_market_name(market.get("marketName", ""))
        home   = event.get("name", "").split(" v ")[0].strip() if " v " in event.get("name", "") else ""
        away   = event.get("name", "").split(" v ")[1].strip() if " v " in event.get("name", "") else ""
        if not home or not away: continue

        fkey = _steam_key(home, away)
        if fkey not in _steam_prices:
            _steam_prices[fkey] = {"home": home, "away": away, "markets": {}}

        market_prices = prices.get(mid, {})
        runners       = list(market_prices.values())

        # Extract prices based on market type
        if mtype == "match_odds" and len(runners) >= 3:
            # runners: home, away, draw (Betfair order)
            _steam_prices[fkey]["markets"].setdefault("draw", {})
            draw_back = runners[2].get("back") if len(runners) > 2 else None
            if draw_back:
                _steam_prices[fkey]["markets"]["draw"][capture_type] = draw_back

        elif mtype == "over_25" and runners:
            # Over 2.5 back price
            o25_back = runners[0].get("back") if runners else None
            _steam_prices[fkey]["markets"].setdefault("over_25", {})
            if o25_back:
                _steam_prices[fkey]["markets"]["over_25"][capture_type] = o25_back

        elif mtype == "over_15" and runners:
            o15_back = runners[0].get("back") if runners else None
            _steam_prices[fkey]["markets"].setdefault("over_15", {})
            if o15_back:
                _steam_prices[fkey]["markets"]["over_15"][capture_type] = o15_back

        elif mtype == "btts" and runners:
            btts_back = runners[0].get("back") if runners else None
            _steam_prices[fkey]["markets"].setdefault("btts", {})
            if btts_back:
                _steam_prices[fkey]["markets"]["btts"][capture_type] = btts_back

        captured += 1

    # Calculate movements where we have both 7am and 5m
    for fkey, data in _steam_prices.items():
        for mkt, mdata in data["markets"].items():
            ...
            if am7 and m5:
                move = round(m5 - am7, 2)
                pct  = round((abs(move) / am7) * 100, 1)
                mdata["move"]     = move
                mdata["move_pct"] = pct
                mdata["steamed"]  = (move < -0.05) and (pct >= 3)  # price shortened = steam

    logger.info(f"✅ Betfair {capture_type} capture complete — {captured} markets processed")
    return _steam_prices

def get_steam_for_fixture(home: str, away: str) -> dict:
    """Get steam detection data for a specific fixture."""
    key = _steam_key(home, away)
    return _steam_prices.get(key, {})

# ── Scheduled capture tasks ──────────────────────────────────────
async def betfair_7am_capture():
    """Runs at 7am every morning — captures opening prices."""
    logger.info("⏰ 7am Betfair price capture starting...")
    try:
        capture_betfair_prices("7am")
        logger.info("✅ 7am capture complete")
    except Exception as e:
        logger.error(f"7am capture error: {e}")

async def betfair_preko_capture():
    """
    Runs every 5 minutes from 12pm onwards.
    Captures prices for matches kicking off in the next 10 minutes.
    """
    try:
        now = datetime.utcnow()
        # Only run between 12pm and 10pm
        if now.hour < 12 or now.hour > 22:
            return
        capture_betfair_prices("5m")
        logger.info(f"✅ Pre-KO capture complete at {now.strftime('%H:%M')}")
    except Exception as e:
        logger.error(f"Pre-KO capture error: {e}")

# ── API Endpoints ────────────────────────────────────────────────
@app.get("/betfair/test")
async def betfair_test():
    """Test Betfair connection — call this after deploying to confirm it works."""
    token = _bf_login()
    if not token:
        return {"status": "❌ Login failed — check BETFAIR_APP_KEY, BETFAIR_USERNAME, BETFAIR_PASSWORD in Railway"}
    markets = _bf_get_football_markets()
    return {
        "status":        "✅ Betfair connected",
        "app_key":       BETFAIR_APP_KEY[:8] + "..." if BETFAIR_APP_KEY else "not set",
        "username":      BETFAIR_USERNAME,
        "markets_found": len(markets),
        "delay_key":     "Yes — 3 minute delay on prices",
        "message":       "Ready to capture prices"
    }

@app.get("/betfair/capture/7am")
async def trigger_7am_capture():
    """Manually trigger 7am price capture — for testing."""
    result = capture_betfair_prices("7am")
    return {
...
@app.get("/betfair/capture/preko")
async def trigger_preko_capture():
    """Manually trigger pre-KO price capture — for testing."""
    result = capture_betfair_prices("5m")
    return {
        "status":    "✅ Pre-KO capture complete",
        "fixtures":  len(result),
        "timestamp": datetime.utcnow().isoformat(),
    }

@app.get("/betfair/steam")
async def get_steam_data():
    """Get all current steam detection data."""
    steams = []
    for fkey, data in _steam_prices.items():
    ...
    return {
        ...
        "steams":  steams,
        "count":   len(steams),
        "all_fixtures": len(_steam_prices),
    }

@app.get("/betfair/prices/{home}/{away}")
async def get_fixture_prices(home: str, away: str):
    """Get captured prices for a specific fixture."""
    data = get_steam_for_fixture(home, away)
    if not data:
        return {"message": f"No price data for {home} v {away} — run /betfair/capture/7am first"}
    return data

logger.info("✅ Betfair price capture loaded — endpoints: /betfair/test /betfair/capture/7am /betfair/capture/preko /betfair/steam")
