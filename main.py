...
# ══════════════════════════════════════════════════════════════
# PERSISTENT DATABASE — PostgreSQL (Railway) or SQLite (fallback)
# ══════════════════════════════════════════════════════════════
...
def _get_db():
    """Get database connection — PostgreSQL if available, else SQLite fallback."""
    if DATABASE_URL:
        try:
            import psycopg2
            ...
            logger.warning("psycopg2 not installed — using SQLite. Add psycopg2-binary to requirements.")
        except Exception as e:
            logger.warning(f"PostgreSQL connection failed: {e} — using SQLite fallback")
    # SQLite fallback — works always
    import sqlite3
    _DB_FILE = "/tmp/accagenius.db"
    ...
    try:
        ...
        for stmt in statements:
            try:
                ...
                logger.warning(f"DB init stmt warning: {e}")
        db.commit()
        db.close()
        logger.info(f"✅ DB initialised ({'PostgreSQL' if _is_postgres() else 'SQLite'})")
    except Exception as e:
...
def db_save_user(email: str, user_data: dict):
    try:
        ...
        if _is_postgres():
            ...
            cur.execute(
                "INSERT OR REPLACE INTO users (email, data, updated_at) VALUES (?, ?, datetime('now'))",
...
def db_load_all_users() -> dict:
    try:
        ...
        if _is_postgres():
            ...
            return {row["email"]: _json.loads(row["data"]) for row in rows}
    except Exception as e:
...
def db_save_acca(acca: dict):
    try:
        ...
        cur = db.cursor()
        aid = str(acca.get("id"))
        email = acca.get("email","")
        data = _json.dumps(acca)
        created = acca.get("created_at", datetime.now().isoformat())
        if _is_postgres():
            cur.execute(
            ...
            cur.execute(
                "INSERT OR REPLACE INTO saved_accas (id, email, data, created_at) VALUES (?, ?, ?, ?)",
...
def db_get_accas(email: str) -> list:
    try:
        ...
        if _is_postgres():
            ...
            cur.execute("SELECT data FROM saved_accas WHERE email=? ORDER BY created_at DESC", (email.lower(),))
        rows = cur.fetchall()
        db.close()
        return [_json.loads(r[0] if _is_postgres() else r["data"]) for r in rows]
    except Exception as e:
...
def db_delete_acca(acca_id: str):
    try:
        with _get_db() as db:
            db.execute("DELETE FROM saved_accas WHERE id=?", (str(acca_id),))
    except Exception as e:
...
def db_save_code(code: str, email: str = "", source: str = "manual"):
    try:
        ...
        if _is_postgres():
            ...
            cur.execute(
                "INSERT OR REPLACE INTO codes (code, email, used_at, source) VALUES (?, ?, ?, ?)",
...
def db_load_codes() -> tuple:
    """Returns (all_codes set, used_codes dict)"""
    try:
        db = _get_db()
        cur = db.cursor()
...
def send_email(to: str, subject: str, html_body: str) -> bool:
    if not RESEND_API_KEY:
        ...
        return False
    reply_to = os.getenv("SUPPORT_EMAIL", "support@accagenius.com")
    try:
        r = requests.post(
...
def _load_user_db():
    """Load users and codes from SQLite (primary) then /tmp (fallback)."""
    global _user_db, _reset_tokens, _used_codes, _all_codes
    import json
    # SQLite first — survives deploys
    try:
        db_users = db_load_all_users()
        if db_users:
            _user_db.update(db_users)
            logger.info(f"✅ Loaded {len(db_users)} users from SQLite")
        db_all, db_used = db_load_codes()
        if db_all:
            _all_codes.update(db_all)
            _used_codes.update(db_used)
            logger.info(f"✅ Loaded {len(db_all)} codes from SQLite")
    except Exception as e:
...
def _load_accas_db():
    """Load saved accas from SQLite (primary) and /tmp (fallback)."""
    ...
    try:
        import json
        with _get_db() as db:
            rows = db.execute("SELECT data FROM saved_accas ORDER BY created_at DESC").fetchall()
            for r in rows:
                acca = _json.loads(r["data"])
                loaded[str(acca.get("id"))] = acca
        logger.info(f"✅ Loaded {len(loaded)} saved accas from SQLite")
    except Exception as e:
        logger.warning(f"SQLite acca load error: {e}")
    # /tmp fallback
    try:
        import json
...
async def billing_portal(request: dict):
    ...
    try:
        ...
        if "url" in portal:
            ...
            return {"error": "Could not create billing session"}
    except Exception as e:
