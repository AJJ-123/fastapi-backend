...
async def register_user(request: dict):
    ...
    # Check if this email was pre-gifted Pro access by admin
    existing = _user_db.get(email, {})
    is_gifted = existing.get("gifted", False) and existing.get("plan") == "pro"

    # Validate access code for pro — skip if user was gifted access
    if plan == "pro" and not is_gifted:
    ...
    # If gifted, force plan to pro regardless of what was submitted
    if is_gifted:
        plan = "pro"
    ...
    _user_db[email] = {
        ...
        "access_code":   existing.get("access_code", "") or access_code,
        "gifted":        existing.get("gifted", False),
        "joined":        existing.get("joined", datetime.now().isoformat()),
        "telegram_invite": TELEGRAM_PRO_INVITE_LINK if plan == "pro" else "",
    }
    if plan == "pro" and access_code and not is_gifted:
    ...
    elif plan == "pro" and is_gifted:
        # Gifted user setting their password — send Telegram invite only
        email_welcome_pro(email, name or existing.get("name",""), "GIFTED", TELEGRAM_PRO_INVITE_LINK)
    else:
