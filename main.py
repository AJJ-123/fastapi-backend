...
LEAGUES = [
    ...
    {"code": "INTF", "name": "International Friendlies","country": "World",       "id": 10,  "flag": "🌍"},
...
async def admin_send_code_email(request: dict):
    ...
    if not ADMIN_PASSWORD_ENV or not token:
        return {"error": "Unauthorised"}
    daily_token = hashlib.sha256((ADMIN_PASSWORD_ENV + datetime.now().strftime("%Y-%m-%d")).encode()).hexdigest()[:32]
    if token != ADMIN_PASSWORD_ENV and token != daily_token:
        return {"error": "Unauthorised"}
    ...
    # Save code to DB so it's valid when customer signs up
    if code not in _all_codes:
        try:
            db_save_code(code, "", "manual")
            _all_codes.add(code)
        except Exception as e:
            logger.error(f"manual code save error: {e}")
...
async def save_acca(request: dict):
    ...
    acca_id = request.get("id") or (len(saved_accas_store) + int(datetime.now().timestamp()))
    acca = {
        "id": acca_id,
        "email": email,
    ...
    # Dedup: skip if same ID already exists for this user
    if any(a.get("id") == acca_id and a.get("email") == email for a in saved_accas_store):
        return {"ok": True, "acca": acca, "duplicate": True}
