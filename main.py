...
FBD_LEAGUE_COUNTRY = {
    "UK1":"England","UK2":"England","UK3":"England","UK4":"England",
...
async def _smart_preko_capture(now: datetime):
    ...
    for fbd in fbd_data:
        ...
        # Only capture in the 4-6 minute window before KO
        if not (4 <= mins <= 6):
            continue
...
async def _generate_continuous_results_sheet() -> bytes:
    ...
    all_history = db_load_all_daily_results(365)  # full season
    ...
    # Flatten all results sorted by date desc then KO
    all_rows = []
    for date_str in sorted(all_history.keys(), reverse=True):
...
async def download_steam_sheet_protected(email: str = ""):
    ...
    try:
        ...
        # ── Steam sheet columns ───────────────────────────────────
        ST_HDRS=[
            "Date","KO Time","Competition","Home Team","Away Team",
        ...
        for i in range(5,10):  ST_H[i]=(S7AM,WHITE)
        for i in range(10,15): ST_H[i]=(S5M,WHITE)
        for i in range(15,20): ST_H[i]=(SFHO,WHITE)
        for i in range(20,25): ST_H[i]=(SO15,WHITE)
        for i in range(25,29): ST_H[i]=(SPOIS,"FFDDBBFF")
        for i in range(29,33): ST_H[i]=(SVAL,AG_GOLD)
        for i in range(33,36): ST_H[i]=(SSYS,WHITE)
        ST_H[36]=(SNOTE,MUTED)

        ST_SECS=[
            (1,5,"MATCH DETAILS"),(6,10,"7AM PRICES"),(11,15,"5 MIN PRE-KO"),
            (16,20,"FH O0.5 MARKETS"),(21,25,"O1.5 MARKETS"),
            (26,29,"POISSON MODEL"),(30,33,"VALUE"),(34,36,"⚠ SYSTEMS"),(37,37,"NOTES"),
        ]
        ...
        for fbd in fbd_data:
            ...
            sys1="TRIGGER" if (dr_steam and o25_steam) else ""
            sys2="TRIGGER" if o25_steam else ""
            sys3="TRIGGER" if fho_steam else ""
            has_steam=any([o25_steam,dr_steam,fho_steam,o15_steam])
        ...
        c2.value=("System 1 LTD: Draw+O2.5 steam  ·  System 2 LU1.5: Back O2.5 steams 3%+  ·  "
                  "System 3 LFHU0.5: Back O1.5 steams 3%+  ·  Prices auto-captured at 7am & 5min pre-KO via Railway")
        ...
        for ri,sr in enumerate(steam_rows,5):
            ...
            row_vals=[
                today.strftime("%d/%m/%Y"),sr["ko"],lg_display,sr["home"],sr["away"],
                sr["o25_7"] or "",sr["o25_5"] or "",o25_mvs,o25_pcts,o25_ss,
                sr["dr_7"] or "", sr["dr_5"] or "", dr_mvs, dr_pcts, dr_ss,
                sr["fho_7"] or "",sr["fho_5"] or "",fho_mvs,fho_pcts,fho_ss,
                sr["o15_7"] or "",sr["o15_5"] or "",o15_mvs,o15_pcts,o15_ss,
                sr["po_o25"],sr["po_draw"],sr["po_o15"],sr["po_fho05"],
                (f"+{sr['val_o25']}%" if sr["val_o25"] and sr["val_o25"]>0 else (f"{sr['val_o25']}%" if sr["val_o25"] else None)),
                (f"+{sr['val_draw']}%" if sr["val_draw"] and sr["val_draw"]>0 else (f"{sr['val_draw']}%" if sr["val_draw"] else None)),
                (f"+{sr['val_o15']}%" if sr["val_o15"] and sr["val_o15"]>0 else (f"{sr['val_o15']}%" if sr["val_o15"] else None)),
                (f"+{sr['val_fho']}%" if sr["val_fho"] and sr["val_fho"]>0 else (f"{sr['val_fho']}%" if sr["val_fho"] else None)),
                sr["sys1"],sr["sys2"],sr["sys3"],None,
            ]
            ...
            for ci,val in enumerate(row_vals,1):
                ...
                if is_steam_col and sv=="STEAM":
                    ...
                    cx.fill=fill("FFFF0000" if "LTD" in ST_HDRS[ci-1] else STEAM_BG)
        ...
        rules=[
            ("SYSTEM 1","LAY THE DRAW (LTD)","TRIGGER: Draw market steams 3%+ AND O2.5 also steams 3%+","ENTRY: Lay the draw at 5min pre-KO price on Betfair Exchange"),
            (None,None,"LOGIC: When draw AND goals steam together = smart money expects goals","EXIT: Green up at HT if 0-0. Let run if goal scored."),
            ("SYSTEM 2","LAY UNDER 1.5 (LU1.5)","TRIGGER: Back O2.5 market steams 3%+","ENTRY: Lay Under 1.5 at 5min pre-KO price"),
            (None,None,"LOGIC: O2.5 steam = smart money backing 3+ goals. Lay the unders.","EXIT: Win if 2+ goals. Lose if 0 or 1 goal."),
            ("SYSTEM 3","LAY FH UNDER 0.5 (LFHU0.5)","TRIGGER: FH O0.5 market steams 3%+","ENTRY: Lay FH Under 0.5 at 5min pre-KO price"),
            (None,None,"LOGIC: FH goals steam = smart money backing first half goal","EXIT: Win if goal in first half. Lose if HT 0-0."),
            (None,None,"STAKING: Start with 1pt until you have 50+ results per system.",
             "Keep records in P&L Tracker. Review after every 25 bets."),
        ]
...
async def _generate_season_pnl_sheet() -> bytes:
    ...
    all_hist = db_load_all_daily_results(90)
    dr_row=3
    for date_str in sorted(all_hist.keys(), reverse=True):
...
async def _trading_scheduler():
    ...
    while True:
        try:
            ...
            logger.info(f"⏰ Trading scheduler: next task in {int(wait_secs/60)}m — {[k for k,v in tasks.items() if v==next_task][0]}")
            await asyncio.sleep(max(min(wait_secs, 60), 30))
            ...
            # 7:15 — store 7am prices from FBD + Odds API
