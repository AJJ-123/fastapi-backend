...
async def get_today_fixtures():
    """All fixtures for today across all leagues — parallel fetching for speed"""
    ...
    with ThreadPoolExecutor(max_workers=17) as executor:
        futures = [loop.run_in_executor(executor, fetch_league_today, league) for league in LEAGUES]
        results = await asyncio.gather(*futures)

    all_matches = [m for sublist in results for m in sublist]

    # Now fetch odds for all fixtures in parallel (batch)
    def fetch_odds_for_match(m: dict) -> dict:
    ...
    # Only fetch odds for non-finished matches (faster)
    live_or_upcoming = [m for m in all_matches if m["status"] not in ["FT", "AET", "PEN"]]
    finished = [m for m in all_matches if m["status"] in ["FT", "AET", "PEN"]]

    if live_or_upcoming:
        with ThreadPoolExecutor(max_workers=20) as executor:
            futures2 = [loop.run_in_executor(executor, fetch_odds_for_match, m) for m in live_or_upcoming]
            live_or_upcoming = list(await asyncio.gather(*futures2))

    all_matches = sorted(live_or_upcoming + finished, key=lambda x: x["time"])
    return {"matches": all_matches, "count": len(all_matches), "date": today}
...
async def generate_acca(request: AccaRequest):
    try:
        leagues = request.leagues if request.leagues else ["PL", "ELC", "PD", "BL1", "SA", "FL1", "CL", "EL"]
        today = datetime.now().date()
        cutoff = today + timedelta(days=3)  # Only next 3 days — relevant, recent xG & form
        all_picks = []
        seen = set()
        ...
        for lc in leagues:
            ...
            if not lid: continue
            # Use `next` param to get upcoming fixtures, filter to 3-day window
            data = api_get("fixtures", {"league": lid, "season": get_season(), "next": 15, "timezone": "Europe/London"})
            fixtures = data.get("response", [])
            # Sort by date ascending so nearest games first
            fixtures.sort(key=lambda f: f["fixture"]["date"])
            for f in fixtures:
                fid = f["fixture"]["id"]
                if fid in seen: continue
                fd = datetime.fromisoformat(f["fixture"]["date"].replace("Z", "+00:00")).date()
                # Strictly within next 3 days
                if fd < today or fd > cutoff:
                    continue
                ...
                if pick and pick["confidence"] >= 55:
                    seen.add(fid)
                    # Include odds bookmaker info in pick
                    odds = get_real_odds(fid)
        ...
        if not picks:
            return {"message": "No picks found for the next 3 days. Try different leagues or market.", "total_selections": 0, "total_odds": 0, "confidence": 0, "selections": []}
        ...
        return {
            "message": f"AI Acca — {request.market.upper()}, {request.risk} risk — Next 3 days only",
