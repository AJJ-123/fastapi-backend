...
# =========================
# ══════════════════════════════════════════════════════════════════
# FOOTBALL-BET-DATA.COM DAILY DOWNLOAD
# Logs in, grabs fresh ASP.NET hidden fields, downloads Excel, parses
# ══════════════════════════════════════════════════════════════════
import io as _io
...
def _fbd_odds_to_prob(val):
    ...
    try:
        v = float(val)
        if v <= 0: return None
        # If value looks like odds (1.0-20.0 range), convert to probability
        if 1.0 <= v <= 20.0:
            return round((1 / v) * 100, 1)
...
def _fbd_get_session():
    """Log into FBD and return authenticated session."""
    import requests as _req
    from bs4 import BeautifulSoup as _BS
    s = _req.Session()
    s.headers.update(_FBD_HEADERS)
    # GET login page for fresh hidden fields
    ...
    logger.info(f"FBD login: VIEWSTATE={'yes' if vs else 'NO'}, EVENTVALIDATION={'yes' if ev else 'NO'}, email_field={'yes' if soup.find('input',{'name':'ctl00$ContentPlaceHolder2$unameTextBox'}) else 'NO'}")
    payload = {
    ...
    r2 = s.post(f"{_FBD_BASE}/signin/",
           data=payload,
           headers={"Content-Type": "application/x-www-form-urlencoded",
                    "Referer": f"{_FBD_BASE}/signin/",
                    "Origin": _FBD_BASE},
           allow_redirects=True, timeout=15)
    # Check if login succeeded
    if "today" in r2.url or "dashboard" in r2.url or "account" in r2.url:
        logger.info(f"FBD login: SUCCESS — redirected to {r2.url}")
    elif "signin" in r2.url:
        logger.warning(f"FBD login: FAILED — still on signin page. Check FBD_EMAIL/FBD_PASSWORD in Railway vars")
    else:
        logger.info(f"FBD login: redirected to {r2.url}")
    return s

def _fbd_download_excel(session):
    """Load /today/, grab fresh hidden fields, POST download button."""
    from bs4 import BeautifulSoup as _BS
    ...
    r2 = session.post(
        f"{_FBD_BASE}/today/", data=payload,
        headers={"Content-Type": "application/x-www-form-urlencoded",
                 "Referer": f"{_FBD_BASE}/today/",
                 "Origin": _FBD_BASE,
    ...
    if not any(x in ct for x in ("excel", "spreadsheet", "octet", "zip")):
        logger.warning(f"FBD unexpected content-type: {ct} — may not be logged in or data not ready yet")
        return None
    return r2.content

def _fbd_parse_excel(excel_bytes):
    """Parse FBD Excel — summary sheet preferred, falls back to all league sheets."""
    try:
        import openpyxl as _xl
        wb = _xl.load_workbook(_io.BytesIO(excel_bytes), read_only=True, data_only=True)
        # Find summary sheet (starts with 'Date-' or named 'summary')
        target = None
        ...
        for row in target.iter_rows(values_only=True):
            ...
            if headers is None:
                if row[0] in ("League","Date","Home Team","col_0") or (len(row) > 1 and row[1] in ("Home Team",)):
...
def get_fbd_data(force=False):
    """Return today's FBD data. Cached per day, auto-refreshes at midnight."""
...
# ── FBD team name aliases — maps API-Football names to FBD names ──
FBD_NAME_ALIASES = {
    # Netherlands
    "psv eindhoven": "psv", "nec nijmegen": "nijmegen", "fortuna sittard": "sittard",
    "az alkmaar": "az", "fc utrecht": "utrecht", "fc twente": "twente",
    "go ahead eagles": "go ahead", "pec zwolle": "zwolle",
    # Germany
    ...
    "tsg hoffenheim": "hoffenheim", "sv werder bremen": "werder bremen",
    ...
    "deportivo alaves": "alaves", "rayo vallecano": "rayo",
    # Italy
    "ac milan": "milan", "inter milan": "inter", "fc internazionale": "inter",
    ...
    "stade rennais": "rennes", "rc lens": "lens",
    # Turkey
    "galatasaray sk": "galatasaray", "fenerbahçe sk": "fenerbahce",
    "beşiktaş jk": "besiktas", "trabzonspor ak": "trabzonspor",
}

def _fbd_normalise(name: str) -> str:
    """Normalise a team name for matching."""
    n = name.lower().strip()
    # Check alias table both ways
    if n in FBD_NAME_ALIASES:
        return FBD_NAME_ALIASES[n]
    # Check reverse (FBD name → API name)
...
def _fbd_names_match(a: str, b: str) -> bool:
    ...
    # One contains the other (PSV contains PSV Eindhoven's normalised form)
    if an in bn or bn in an: return True
    # Any significant word (>3 chars) matches
...
def get_fbd_match(home: str, away: str):
    """Match a fixture against today's FBD data using multi-strategy name matching."""
...
def fbd_boost_confidence(base_conf: int, home: str, away: str, bet: str) -> int:
    """
    Boost (or reduce) confidence score using FBD prediction data.
    bet = 'home' | 'draw' | 'away'
    """
    fbd = get_fbd_match(home, away)
    if not fbd: return base_conf
    pred = fbd.get("prediction", "").strip().upper()
    # FBD agrees with our pick — boost by up to 8 points
    ...
    # FBD disagrees — reduce by 5
    if bet == "home"  and pred in ("A","2"): base_conf = max(30, base_conf - 5)
    if bet == "away"  and pred in ("H","1"): base_conf = max(30, base_conf - 5)
    # Value signal: our pick's predicted odds > market avg = value bet
    if bet == "home":
        ph = fbd.get("pred_home_odds"); ah = fbd.get("avg_home_odds")
        if ph and ah and ph > ah * 1.05: base_conf = min(95, base_conf + 4)
    if bet == "away":
        pa = fbd.get("pred_away_odds"); aa = fbd.get("avg_away_odds")
        if pa and aa and pa > aa * 1.05: base_conf = min(95, base_conf + 4)
    return base_conf
        ...
        try:
            ...
            if hour == 9 and acca_sent_date != today:
                ...
                try:
                    ...
                    # ── FBD Value Bets: BTTS + Over 2.5 picks from today's data ──
                    try:
                        ...
                        for m in fbd_matches:
                            ...
                            # BTTS: FBD pred_btts probability > 60%
                            btts = m.get("pred_btts")
                            if btts and float(btts) >= 60:
                                avg_btts = m.get("avg_btts_yes")
                                odds_str = f"@ ~{avg_btts:.2f}" if avg_btts and avg_btts > 0 else ""
                                btts_picks.append(f"  ⚽ {home} vs {away} — BTTS {odds_str} (prob {int(btts)}%)")
                            # Over 2.5: FBD pred_over25 probability > 60%
                            ov25 = m.get("pred_over25")
                            if ov25 and float(ov25) >= 60:
                                avg_ov = m.get("avg_over25")
                                odds_str = f"@ ~{avg_ov:.2f}" if avg_ov and avg_ov > 0 else ""
                                over25_picks.append(f"  ⚽ {home} vs {away} — Over 2.5 {odds_str} (prob {int(ov25)}%)")

                        sep = "━" * 20
                        if btts_picks and has_pro:
                            btts_msg = (
                                "🎯 <b>TODAY'S BTTS VALUE PICKS</b>\n"
                                + sep + "\n"
                                + "\n".join(btts_picks[:5]) + "\n"
                                + sep + "\n"
                                + "📊 Powered by FBD prediction model\n"
                                + "⚠️ For info only — 18+ | BeGambleAware.org\n"
                                + "🤖 AccaGenius · accagenius.com"
                            )
                            send_telegram_to(btts_msg, pro_channel)

                        if over25_picks and has_pro:
                            ov25_msg = (
                                "📈 <b>TODAY'S OVER 2.5 VALUE PICKS</b>\n"
                                + sep + "\n"
                                + "\n".join(over25_picks[:5]) + "\n"
                                + sep + "\n"
                                + "📊 Powered by FBD prediction model\n"
                            ...
                            send_telegram_to(ov25_msg, pro_channel)

                        if (btts_picks or over25_picks):
                            logger.info(f"9am value bets sent: {len(btts_picks)} BTTS, {len(over25_picks)} Over 2.5")
...
async def startup_event():
    ...
    if FBD_EMAIL and FBD_PASSWORD:
        ...
        print(f"FBD: ❌ FBD_EMAIL='{FBD_EMAIL}' FBD_PASSWORD='{'set' if FBD_PASSWORD else 'MISSING'}' — check Railway vars", flush=True)
    print("AccaGenius API started", flush=True)


async def _fbd_startup_download():
    """Download FBD data on startup."""
...
async def fbd_daily_scheduler():
    """
    Downloads FBD data every morning at 08:15 UK time.
    Runs as a background task — no manual intervention needed.
    If FBD_EMAIL / FBD_PASSWORD not set, sleeps and retries tomorrow.
    """
    import pytz
    ...
    while True:
        try:
            ...
            # Target: 08:15 today (after FBD updates at 08:00-09:00)
            ...
            if FBD_EMAIL and FBD_PASSWORD:
                logger.info("🔄 FBD: starting daily download...")
                data = get_fbd_data(force=True)
                logger.info(f"✅ FBD: {len(data)} matches loaded for today")
            else:
                logger.warning("FBD: FBD_EMAIL/FBD_PASSWORD not set — skipping download")
        except Exception as e:
...
def analyze_and_pick_with_fbd(fixture, home_form, away_form, risk, market="winner"):
    ...
    If FBD has no data for this match, returns the original pick unchanged.
    """
    ...
    try:
        ...
        if "draw" in bet_lower:
            side = "draw"
        elif away.lower() in bet_lower:
            side = "away"
        else:
            side = "home"
...
@app.get("/fbd/today")
async def fbd_today(token: str = ""):
    """Return today's FBD data — admin only."""
...
async def fbd_refresh(request: dict):
    """Force re-download of today's FBD file — admin only."""
...
async def fbd_value_bets():
    """Return today's BTTS and Over 2.5 value picks from FBD data — Pro endpoint."""
    data = get_fbd_data()
    btts = []
    over25 = []
    for m in data:
        ...
        if not home or not away: continue
        bp = m.get("pred_btts")
        op = m.get("pred_over25")
        if bp and float(bp) >= 55:
            btts.append({
                "home": home, "away": away,
                "league": m.get("league",""),
                "btts_prob": bp,
                "avg_odds": m.get("avg_btts_yes"),
    ...
    return {
        ...
        "btts": btts[:10],
        "over25": over25[:10],
        "fbd_loaded": len(data) > 0,
        "fbd_match_count": len(data)
    }
...
async def get_today_fixtures():
    """
    FBD-first fixture list.
    1. Load today's FBD file (142 matches) as the master list
    2. Enrich each FBD match with API-Football data (logos, kick times, live scores, odds)
    3. Add any API matches not in FBD (CL, internationals etc) at the end
    Cached 10 minutes.
    """
    today_str = _london_now().strftime("%Y-%m-%d")
    ...
    # ── Step 1: Load FBD as master fixture list ──
    fbd_data = get_fbd_data()

    # ── Step 2: Fetch all API fixtures in parallel ──
    ...
    with ThreadPoolExecutor(max_workers=34) as executor:
        ...
        api_results = await asyncio.gather(*futures, return_exceptions=True)

    # Flatten all API fixtures into one list
    api_fixtures = []
    for r in api_results:
        if isinstance(r, list):
            api_fixtures.extend(r)

    # ── Step 3: Build API lookup by team name ──
    def build_api_match(f: dict) -> dict:
        """Convert raw API fixture to enriched match dict."""
        try:
            status   = f["fixture"]["status"]["short"]
            kick     = datetime.fromisoformat(f["fixture"]["date"].replace("Z","+00:00")).strftime("%H:%M")
            return {
                "id":          f["fixture"]["id"],
                "time":        kick,
                "home":        f["teams"]["home"]["name"],
    ...
    # ── Step 4: Build merged fixture list ──
    all_matches = []
    used_api_ids = set()
    ...
    if fbd_data:
        for fbd in fbd_data:
            ...
            for am in api_matches:
                if _fbd_names_match(home, am["home"]) and _fbd_names_match(away, am["away"]):
                    api_m = am
                    break

            if api_m:
                used_api_ids.add(api_m["id"])
                # Use API fixture enriched with FBD data
                merged = dict(api_m)
            else:
                # FBD-only fixture (no API match found)
                merged = {
            ...
            # Always attach FBD intelligence
            merged["fbd_prediction"] = fbd.get("prediction","")
            merged["fbd_btts"]       = fbd.get("pred_btts")
            merged["fbd_over25"]     = fbd.get("pred_over25")
            merged["fbd_pred_goals"] = fbd.get("pred_goals")
            merged["fbd_avg_h_odds"] = fbd.get("avg_home_odds")
            merged["fbd_avg_a_odds"] = fbd.get("avg_away_odds")
            merged["fbd_source"]     = True
            all_matches.append(merged)

        # Add API-only matches (CL, internationals etc not in FBD)
        for am in api_matches:
            if am["id"] not in used_api_ids:
                am["fbd_source"] = False
                all_matches.append(am)
    else:
        # No FBD data — fall back to pure API
        all_matches = api_matches

    all_matches.sort(key=lambda x: x.get("time","99:99"))

    # ── Step 5: Fetch odds for upcoming matches ──
    def fetch_odds_today(m: dict) -> dict:
        if m.get("status") not in ["NS","TBD"] or not m.get("id"): return m
        try:
    ...
    ns_matches = [m for m in all_matches if m.get("status") in ("NS","TBD") and m.get("id")][:20]
    other_matches = [m for m in all_matches if m.get("status") not in ("NS","TBD") or not m.get("id")]
    ...
    try:
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            ns_matches = list(await asyncio.wait_for(
                asyncio.gather(*futures_odds, return_exceptions=True), timeout=15))
            ns_matches = [m for m in ns_matches if isinstance(m, dict)]
    except asyncio.TimeoutError:
        logger.warning("Odds fetch timed out")

    remaining = [m for m in all_matches if m.get("status") in ("NS","TBD") and m.get("id") not in ns_ids]
    all_matches = sorted(other_matches + ns_matches + remaining, key=lambda x: x.get("time","99:99"))

    # Strip _raw before caching (too large)
    for m in all_matches:
        m.pop("_raw", None)

    fbd_count = sum(1 for m in all_matches if m.get("fbd_source"))
    logger.info(f"/today: {len(all_matches)} matches ({fbd_count} FBD-matched, {len(all_matches)-fbd_count} API-only)")
...
async def get_today_acca():
    ...
    # FBD Value acca — only FBD matches, data-driven
    fbd_req = AccaRequest(selections=10, risk="balanced", leagues=leagues, market="winner", today_only=True)
    try:
        fbd_result  = await generate_fbd_value_acca(fbd_req)
        fbd_picks   = fbd_result.get("selections", [])
    except Exception as e:
        logger.error(f"today-acca fbd error: {e}")
        fbd_picks = []

    # Filter form/rank picks to only include FBD-covered matches
    fbd_data = get_fbd_data()
    def is_fbd_match(p):
        return any(_fbd_names_match(p.get("home",""), m["home"]) and 
                   _fbd_names_match(p.get("away",""), m["away"]) for m in fbd_data)

    form_picks  = [p for p in form_picks  if is_fbd_match(p)] if fbd_data else form_picks
    rank_picks  = [p for p in rank_picks  if is_fbd_match(p)] if fbd_data else rank_picks
    ...
    result = {
        ...
        "accas": [
            ...
            {"theme": "fbd_value", "label": "FBD Value 4-Fold", "emoji": "💎", "picks": t3},
        ]
    }
...
async def generate_acca(request: AccaRequest):
    try:
        # ── Use FBD league codes if FBD data is loaded — much faster, better quality ──
        fbd_data = get_fbd_data()
        if fbd_data and not request.leagues:
            # Extract unique league codes from today's FBD file
            fbd_leagues = list(dict.fromkeys(
                m.get("league","") for m in fbd_data if m.get("league","")
            ))
            # Map FBD league codes to our internal codes
            FBD_TO_INTERNAL = {
                "E0":"PL","E1":"ELC","E2":"EL1","E3":"EL2","EC":"EC","ECN":"ECN","ECS":"ECS",
                "D1":"BL1","D2":"BL2","SP1":"PD","SP2":"SP2","IT1":"SA","IT2":"IT2",
                "FR1":"FL1","FR2":"FL2","N1":"NED","B1":"BEL","B2":"BEL",
                "SC1":"SPFL","SC2":"SC2","SC3":"SC3","T1":"TUR","T2":"TUR",
                "PT1":"PPL","PT2":"PT2","G1":"GRE","SA1":"SA1","WA1":"WA1",
                "MX1":"MEX","PA1":"PAR","N1":"NED",
            }
            leagues = list(set(
                FBD_TO_INTERNAL.get(lc, lc) for lc in fbd_leagues
                if FBD_TO_INTERNAL.get(lc, lc) in LEAGUE_IDS
            ))
            if not leagues:
                leagues = request.leagues if request.leagues else ["PL","ELC","PD","BL1","SA","FL1","NED","TUR","PPL","BEL","SPFL"]
        else:
            leagues = request.leagues if request.leagues else ["PL","ELC","PD","BL1","SA","FL1","NED","TUR","PPL","BEL","SPFL"]
        ...
        def fetch_league_picks(lc: str) -> list:
            ...
            try:
                ...
                picks_this_league = 0
                fbd_today = get_fbd_data()  # cached — no extra cost
                for f in fixtures:
                    ...
                    # ── FBD filter: only pick games in today's FBD file ──
                    if fbd_today and not any(
                        _fbd_names_match(home_name, m["home"]) and _fbd_names_match(away_name, m["away"])
                        for m in fbd_today
                    ):
                        continue
    ...
    try:
        ...
        for lc in leagues:
            ...
            for f in fixtures:
                ...
                # FBD filter for today's games
                if fd == today and fbd_today:
...
async def generate_btts_acca(request: AccaRequest):
    """BTTS acca — powered by FBD predicted BTTS probability. Falls back to API form stats if FBD unavailable."""
    try:
        today     = _london_now().date()
        ...
        if fbd_data:
            # FBD-powered BTTS picks
            cached_today = cache_get(f"today_{today_str}") or {}
            api_matches  = {
                f"{m.get('home','').lower()}|{m.get('away','').lower()}": m
            ...
            for fbd in fbd_data:
                ...
                if not home or not away or not btts_prob: continue
                if float(btts_prob) < 55: continue

                # Find API match for logos/ID
                api_m = None
                for am in cached_today.get("matches", []):
                    if _fbd_names_match(home, am.get("home","")) and _fbd_names_match(away, am.get("away","")):
                        api_m = am; break

                fix_id = api_m["id"] if api_m else abs(hash(f"{home}{away}{today_str}")) % 9999999
                if fix_id in seen: continue

                odds = get_real_odds(fix_id) if api_m else {}
                conf = min(90, 50 + int(float(btts_prob) * 0.5))
                ...
                all_picks.append({
                    ...
                    "fbd_btts":     btts_prob,
                    "fbd_source":   True,
                    "reasoning":    f"FBD BTTS probability: {btts_prob:.0f}% · FBD prediction: {fbd.get('prediction','')}",
                })
                seen.add(fix_id)
        else:
            # Fallback: API form-based BTTS
            leagues = request.leagues if request.leagues else ["PL","ELC","BL1","SA","FL1","PD"]
            cutoff  = today + timedelta(days=3)
            random.shuffle(leagues)
            def fetch_btts_api(lc: str) -> list:
                lid = LEAGUE_IDS.get(lc)
                if not lid: return []
                picks = []
                try:
                    ...
                    for f in sorted(data.get("response", []), key=lambda x: x["fixture"]["date"]):
                        ...
                        hf = get_team_form(f["teams"]["home"]["id"], lid)
                        af = get_team_form(f["teams"]["away"]["id"], lid)
                        combined_btts = round((hf.get("btts_pct",0) + af.get("btts_pct",0)) / 2)
                        if combined_btts >= 55:
                            ...
                            picks.append({
                                ...
                                "bet": "Both Teams to Score", "market_type": "BTTS",
                                "odds": odds.get("btts", 1.72), "odds_home": odds.get("home",0),
                                "odds_draw": odds.get("draw",0), "odds_away": odds.get("away",0),
                                "odds_available": odds.get("available", False),
                                "confidence": min(88, 55 + int(combined_btts * 0.4)),
                                "reasoning": f"Combined BTTS% {combined_btts}%",
                            })
                except Exception: pass
                return picks
            with ThreadPoolExecutor(max_workers=10) as ex:
                loop = asyncio.get_event_loop()
                fb_results = await asyncio.gather(*[loop.run_in_executor(ex, fetch_btts_api, lc) for lc in leagues])
            for lp in fb_results:
                for pick in lp:
                    if pick["id"] not in seen:
                        seen.add(pick["id"]); all_picks.append(pick)
...
async def generate_fbd_value_acca(request: AccaRequest):
    """
    FBD Value Acca — picks only from games in today's FBD daily file.
    Uses FBD predicted odds vs market odds to identify value bets.
    Falls back to FBD prediction (H/D/A) + highest confidence from form data.
    """
    try:
        fbd_data = get_fbd_data()
        if not fbd_data:
            return {"message": "FBD data not loaded yet — try again shortly.", 
                    "total_selections": 0, "total_odds": 0, "confidence": 0, "selections": []}
        ...
        # Load today's fixtures from cache
        cached_today = cache_get(f"today_{today_str}")
        today_fixtures = {}
        if cached_today:
            for m in cached_today.get("matches", []):
                key = f"{m.get('home','').lower()}|{m.get('away','').lower()}"
                today_fixtures[key] = m
        ...
        for fbd in fbd_data:
            ...
            pred       = fbd.get("prediction","").strip().upper()  # H/D/A
            # These are stored as probabilities (%) after _fbd_odds_to_prob conversion
            # Convert back to decimal odds: odds = 100 / probability
            ...
            ph         = prob_to_odds(fbd.get("pred_home_odds"))   # predicted home odds
            pa         = prob_to_odds(fbd.get("pred_away_odds"))    # predicted away odds
            ah         = prob_to_odds(fbd.get("avg_home_odds"))     # market avg home odds
            aa         = prob_to_odds(fbd.get("avg_away_odds"))     # market avg away odds
            btts_prob  = fbd.get("pred_btts")    # already a probability %
            ov25_prob  = fbd.get("pred_over25")  # already a probability %

            if not pred: continue

            # Find matching API fixture for logos/IDs
            api_fix = None
            for key, m in today_fixtures.items():
                if _fbd_names_match(home, m.get("home","")) and _fbd_names_match(away, m.get("away","")):
                    api_fix = m
                    break

            fix_id  = api_fix["id"]   if api_fix else abs(hash(f"{home}{away}{today_str}")) % 999999
            league  = api_fix.get("league", fbd.get("league","")) if api_fix else fbd.get("league","")
            hl      = api_fix.get("home_logo","") if api_fix else ""
            al      = api_fix.get("away_logo","") if api_fix else ""
            kick    = api_fix.get("time","TBC") if api_fix else "TBC"

            if fix_id in seen: continue

            # --- Value detection ---
            # 1. FBD predicted odds significantly higher than market (value bet)
            # 2. Clear directional prediction
            value_score = 0
            bet     = None
            odds    = None
            market  = "1X2"
            ...
            if pred == "H":
                ...
                value_score += 10
                reason_parts.append(f"FBD predicts HOME win")
            elif pred == "A":
                bet = f"{away} Win"
                ...
                value_score += 10
                reason_parts.append(f"FBD predicts AWAY win")
            elif pred == "D":
                bet = "Draw"
            ...
            # Boost for high BTTS or Over 2.5 probability
            if btts_prob and btts_prob >= 60:
                value_score += 5
                reason_parts.append(f"BTTS {btts_prob:.0f}%")
            if ov25_prob and ov25_prob >= 60:
                value_score += 5
                reason_parts.append(f"Over 2.5 {ov25_prob:.0f}%")

            if not bet or value_score < 10: continue

            # Clamp odds to reasonable range
            if odds: odds = max(1.30, min(float(odds), 8.0))
            else: odds = 1.90

            conf = min(88, 52 + value_score)
            ...
            picks.append({
                ...
                "time":         kick,
                "league":       league,
                "league_id":    api_fix.get("league_id", 0) if api_fix else 0,
                "bet":          bet,
                "market_type":  market,
                "odds":         round(odds, 2),
                "confidence":   conf,
...
async def stripe_webhook(request):
    ...
    # Trial converted to paid subscription
        ...
        if cust_email and cust_email in _user_db:
            if status == "active" and _user_db[cust_email].get("plan_status") == "trial":
                ...
                send_email(cust_email, "⚡ AccaGenius Pro — Payment Confirmed",
                    _email_base(f"<p>Hi {first},</p><p>Your AccaGenius Pro subscription is now active. £20/month will be charged on the same date each month.</p><p>Keep enjoying your Pro access! ⚡</p>"))
    ...
    # Subscription cancelled or payment failed — downgrade to free
        ...
        if cust_email and cust_email in _user_db:
            ...
            first = _user_db[cust_email].get("name", "there")
            send_email(cust_email, "Your AccaGenius Pro subscription has ended",
                _email_base(f"<p>Hi {first},</p><p>Your AccaGenius Pro subscription has ended. You still have free access to AccaGenius.</p><p>To rejoin Pro at any time, visit <a href='{SITE_URL}' style='color:#f0b429'>{SITE_URL}</a> and upgrade again.</p><p>We'd love to have you back ⚡</p>"))
...
async def delete_saved_acca(request: dict):
    ...
    global saved_accas_store
    # Remove from memory
    saved_accas_store = [a for a in saved_accas_store
                         if str(a.get("id","")) != acca_id]
    # Delete from DB permanently
    try:
        db_delete_acca(acca_id)
    except Exception as e:
        logger.warning(f"delete_saved_acca DB error: {e}")
    # Update /tmp backup
    try:
        import json
        with open(_ACCAS_DB_FILE, 'w') as f:
    ...
    logger.info(f"Permanently deleted acca {acca_id} for {email}")
    return {"ok": True, "message": f"Acca {acca_id} deleted"}


@app.post("/saved-accas/reset")
async def reset_saved_accas(request: dict):
    """Clear all saved accas for a user — permanently from DB and memory."""
    email = (request.get("email") or "").strip().lower()
    if not email:
    ...
    ids_to_delete = [str(a.get("id")) for a in saved_accas_store if a.get("email","").lower() == email]
    ...
    try:
        ...
        if pg:
            ...
            for aid in ids_to_delete:
                cur.execute("DELETE FROM saved_accas WHERE id=%s", (aid,))
            pg.commit(); pg.close()
        else:
            conn = _get_sqlite()
            for aid in ids_to_delete:
                conn.execute("DELETE FROM saved_accas WHERE id=?", (aid,))
            conn.commit(); conn.close()
    except Exception as e:
        logger.warning(f"DB reset accas error: {e}")
    ...
    logger.info(f"Permanently deleted {len(ids_to_delete)} accas for {email}")
    return {"ok": True, "message": f"All {len(ids_to_delete)} accas permanently deleted"}
