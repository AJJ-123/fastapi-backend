    ...
    try:
        ...
        for row in target.iter_rows(values_only=True):
            ...
            matches.append({
                ...
                "home":            home,
                "away":            away,
                "prediction":      str(m.get("Pred'n","") or m.get("Prediction","")),
                "pred_home_odds":  _fbd_safe_float(m.get("Pred H Odds")),
                "pred_draw_odds":  _fbd_safe_float(m.get("Pred D Odds")),
                "pred_away_odds":  _fbd_safe_float(m.get("Pred A Odds")),
                "avg_home_odds":   _fbd_safe_float(m.get("Avg H Odds")),
                "avg_away_odds":   _fbd_safe_float(m.get("Avg A Odds")),
                "max_home_odds":   _fbd_safe_float(m.get("Max H Odds")),
                "pred_over25":     _fbd_safe_float(m.get("Pred Ov25")),
                "pred_under25":    _fbd_safe_float(m.get("Pred Un25")),
                "pred_btts":       _fbd_safe_float(m.get("Pred BTS Y")),
                "pred_goals":      _fbd_safe_float(m.get("Pred Tot Gls")),
                "avg_over25":      _fbd_safe_float(m.get("Avg ov25")),
                "avg_btts_yes":    _fbd_safe_float(m.get("BTS Y")),
                "dc_1x":           _fbd_safe_float(m.get("DC 1X")),
                "dc_x2":           _fbd_safe_float(m.get("DC X2")),
            })
...
def get_fbd_match(home: str, away: str):
    """Fuzzy-match a fixture against today's FBD data."""
    hl, al = home.lower(), away.lower()
    for m in get_fbd_data():
        mh, ma = m["home"].lower(), m["away"].lower()
        if (hl in mh or mh in hl) and (al in ma or ma in al):
            return m
    return None
...
async def startup_event():
    ...
    _init_db()          # create SQLite tables
    _load_paper_db()
    _load_user_db()     # loads from SQLite + /tmp fallback
    _load_accas_db()    # loads from SQLite + /tmp fallback
    _load_referrals()
    asyncio.create_task(live_alert_scanner())
    asyncio.create_task(fbd_daily_scheduler())
    # Trigger FBD download immediately on startup if credentials set
    if FBD_EMAIL and FBD_PASSWORD:
        logger.info(f"FBD: credentials found ({FBD_EMAIL}) — triggering startup download...")
        asyncio.create_task(_fbd_startup_download())
    else:
        logger.warning("FBD: FBD_EMAIL or FBD_PASSWORD not set in Railway variables — skipping FBD")
    logger.info("AccaGenius API started — Telegram scanner + FBD scheduler running")


async def _fbd_startup_download():
    """Download FBD data on startup — runs in background so it doesn't delay server start."""
    await asyncio.sleep(5)  # wait 5s for server to fully start
    try:
        data = get_fbd_data(force=True)
        logger.info(f"✅ FBD startup download: {len(data)} matches loaded")
    except Exception as e:
        logger.error(f"FBD startup download failed: {e}")
...
async def get_today_fixtures():
    ...
    # ── FBD fallback: if API returned nothing, build fixture list from FBD daily file ──
    if not all_matches:
        try:
            ...
            if fbd_data:
                ...
                for m in fbd_data:
                    ...
                    if not home or not away: continue
                    fbd_matches.append({
                        "id": abs(hash(f"{home}{away}{today_str}")) % 999999,
                        "time": "TBC",
                        "home": home,
                        "away": away,
                        "home_id": 0,
                        ...
                        "avg_home_odds": m.get("avg_home_odds"),
                        "avg_away_odds": m.get("avg_away_odds"),
                    })
                if fbd_matches:
                    logger.info(f"/today: API empty — using FBD fallback ({len(fbd_matches)} matches)")
                    result = {"matches": fbd_matches, "count": len(fbd_matches), "date": today_str,
                              "leagues_failed": [], "fbd_source": True}
    ...
    if all_matches and len(still_failed) <= 1:
        ...
        cache_set(cache_key, result, 600)  # partial — retry soon
